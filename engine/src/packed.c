...
/* Exported for packed_simd.c; declared in packed_internal.h. */
...
xaios_engine_status_t xaios_packed_matrix_validate(
  ...
  status = xaios_packed_matrix_required_bytes(elements, matrix->dtype,
                                              &required_data);
  if (status != XAIOS_ENGINE_OK) return status;
  status = checked_multiply(matrix->rows, xaios_packed_group_count(matrix),
                            &required_scales);
  if (status != XAIOS_ENGINE_OK) return status;
...
/* xaios_packed_gemv_sve and xaios_packed_gemv_neon live in packed_simd.c.
   They keep their names and signatures from packed.h. */
...
/* Exported for the AVX2 wrapper in packed_simd.c; declared in
   packed_internal.h. */
...
/* packed_gemv_avx2_impl and xaios_packed_gemv_avx2 live in packed_simd.c.
   They keep their names and signatures from packed.h. */
...
int xaios_packed_avx2_available(void) {
  ...
  int observed = __atomic_load_n(&cached, __ATOMIC_ACQUIRE);
  if (observed != 0) return observed > 0;
  if (!xaios_packed_x86_avx2_usable()) {
