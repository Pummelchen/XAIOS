#ifndef XAIOS_ENGINE_MODEL_VOLUME_H
#define XAIOS_ENGINE_MODEL_VOLUME_H
...
#define XAIOS_MODEL_VOLUME_MAGIC "XAIOSV1\0"
#define XAIOS_MODEL_VOLUME_SUPERBLOCK_SIZE UINT64_C(4096)
#define XAIOS_MODEL_VOLUME_CATALOG_HEADER_SIZE UINT64_C(256)
#define XAIOS_MODEL_VOLUME_PACKAGE_RECORD_SIZE UINT64_C(384)
#define XAIOS_MODEL_VOLUME_CHUNK_RECORD_SIZE UINT64_C(128)
#define XAIOS_MODEL_VOLUME_DATA_START UINT64_C(1048576)
#define XAIOS_MODEL_VOLUME_PACKAGE_STAGING UINT32_C(1)
#define XAIOS_MODEL_VOLUME_PACKAGE_ACTIVE UINT32_C(2)
#define XAIOS_MODEL_VOLUME_PACKAGE_QUARANTINED UINT32_C(3)
#define XAIOS_MODEL_VOLUME_CHUNK_COMPLETE UINT32_C(1)
#define XAIOS_MODEL_VOLUME_CHUNK_ZERO UINT32_C(2)
#define XAIOS_MODEL_VOLUME_CHUNK_FREE UINT32_C(4)
#define XAIOS_MODEL_VOLUME_CHUNK_HASH_PENDING UINT32_C(8)

typedef xaios_model_v2_reader_t xaios_model_volume_reader_t;

typedef xaios_engine_status_t (*xaios_model_volume_verify_signature_fn)(
    void *context, const uint8_t public_key[32], const uint8_t signature[64],
    const uint8_t message[32]);

typedef xaios_engine_status_t (*xaios_model_volume_write_at_fn)(
    void *context, uint64_t offset, const void *source, size_t length);
typedef xaios_engine_status_t (*xaios_model_volume_flush_fn)(void *context);

typedef struct xaios_model_volume_writer {
  void *context;
  xaios_model_volume_write_at_fn write_at;
  xaios_model_volume_flush_fn flush;
} xaios_model_volume_writer_t;

typedef struct xaios_model_volume {
...
} xaios_model_volume_t;

typedef struct xaios_model_volume_package {
  uint32_t state;
  uint64_t record_id;
  ...
  char target_id[33];
} xaios_model_volume_package_t;

typedef struct xaios_model_volume_chunk {
  uint64_t record_id;
  ...
  uint32_t flags;
  uint8_t checksum[32];
} xaios_model_volume_chunk_t;

typedef struct xaios_model_volume_probe {
