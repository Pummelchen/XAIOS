"""On-disk model-v2 foundations: constants, sources, writer and header reader.

This module is the private foundation of ``xaios_model_v2``. It owns the
``xaios.model.v2`` constant table, the streaming section sources, the
``PackageWriter`` that lays sections and tensors out deterministically, and
``read_header``. The ``xaios_model_v2`` entry point imports these names and
re-exports them, so the command line and ``PYTHONPATH=tools`` importers keep
using a single module for the package format.
"""
