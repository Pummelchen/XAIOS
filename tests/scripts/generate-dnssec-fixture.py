#!/usr/bin/env python3
"""Generate bounded signed DNSSEC test chains for the C verifier.

Two targets, because two different builds need a chain they control:

  hosted  -- build/hosted/dnssec_fixture.h, regenerated by `make hosted-test`
             on every run and never committed.
  kernel  -- kernel/net/dns_selftest_chain.h, generated once and committed,
             because dns_self_test() runs inside the kernel on a machine
             with no Python and no network. Regenerating it is a deliberate
             act; the chain is self-contained (the trust anchor it is
             checked against travels with it), so a stale copy cannot
             silently start passing against the real root.
"""
...
def kernel_selftest(output: Path) -> None:
    """Emit the chain dns_self_test() walks inside the kernel.

    RSA/SHA-256 rather than one of the elliptic algorithms: the kernel runs
    this on every boot, including under TCG emulation, and a 1024-bit RSA
    verification with exponent 65537 is the cheapest signature check
    BearSSL offers. Algorithm coverage belongs to the hosted test, which
    pays for it once on a real CPU.
    """
    ...
    child_ds = (key_tag(child_key).to_bytes(2, "big") + b"\x08\x02" +
                hashlib.sha256(wire_name(zone) + child_key).digest())
    root_message = message("", 48, [
        ("", 48, 3600, root_key),
        ("", 46, 3600, rrsig("", 48, 3600, root_key, "", root, 8, root_key)),
    ])
    ds_message = message(zone, 43, [
        (zone, 43, 3600, child_ds),
        (zone, 46, 3600, rrsig(zone, 43, 3600, child_ds, "", root, 8,
                               root_key)),
    ])
    dnskey_message = message(zone, 48, [
        (zone, 48, 3600, child_key),
        (zone, 46, 3600, rrsig(zone, 48, 3600, child_key, zone, child, 8,
                               child_key)),
    ])
    a_rdata = bytes((10, 53, 0, 7))
    a_message = message(zone, 1, [
        (zone, 1, 60, a_rdata),
        (zone, 46, 60, rrsig(zone, 1, 60, a_rdata, zone, child, 8,
                             child_key)),
    ])
    aaaa_rdata = bytes.fromhex("20010db8000000000000000000000035")
    aaaa_message = message(zone, 28, [
        (zone, 28, 60, aaaa_rdata),
        (zone, 46, 60, rrsig(zone, 28, 60, aaaa_rdata, zone, child, 8,
                             child_key)),
    ])
    source = (
    ...
    for name, value in (("k_selftest_root_dnskey", root_message),
                        ("k_selftest_ds", ds_message),
                        ("k_selftest_dnskey", dnskey_message),
                        ("k_selftest_a", a_message),
                        ("k_selftest_aaaa", aaaa_message),
                        ("k_selftest_anchor_digest",
                         hashlib.sha256(wire_name("") + root_key).digest()),
                        ("k_selftest_a_rdata", a_rdata),
                        ("k_selftest_aaaa_rdata", aaaa_rdata)):
        source += c_array(name, value)
    source += f"#define DNS_SELFTEST_ROOT_TAG {key_tag(root_key)}U\n"
    source += '#define DNS_SELFTEST_ZONE "selftest"\n'
    source += "#endif\n"
...
def main() -> None:
    ...
    parser.add_argument("--target", choices=("hosted", "kernel"),
                        default="hosted")
    args = parser.parse_args()
    if args.target == "kernel":
        kernel_selftest(args.output)
        return
