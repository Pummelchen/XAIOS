#!/usr/bin/env python3
"""Console and session correlation for vmware-fusion-load-soak.py.

Moved verbatim out of the soak so that neither half has to carry the whole
run. This half is everything that reads the guest's console as evidence: the
free-memory telemetry reader, the session-marker filter, the positional
console cursor, the accepted/closed socket correlation and the `--self-test`
that exercises the correlation on a machine with no hypervisor.

It is imported by the driver and is not itself a program: `python3
tests/scripts/vmware-fusion-load-soak.py` remains the only entry point, with
the same command line, output and exit codes.
"""
