#!/usr/bin/env python3
"""QEMU discovery and CPU-tier probes for the CPU matrix gate.

The gate is `qemu-cpu-matrix.py`. This module holds the half that talks to an
emulator -- finding one, asking which CPU models it can model, watching a boot
for the markers its tier is held to, and the three per-architecture tier
runners -- split out so the gate, which sequences the tiers and writes the
report, stays under the repository's 500-line limit. Nothing below changed in
the move; the gate imports these names, so the command line, the output and
the exit codes are unchanged.

It is imported and is not itself a program: `python3
tests/scripts/qemu-cpu-matrix.py` remains the only entry point.
"""
