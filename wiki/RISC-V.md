...
| `python3 tests/scripts/qemu-operations-closure.py --arch riscv64` | The administrative lifecycle over SSH on this architecture, the same one the other two run: a durable record is left by stopping QEMU abruptly, the next boot is told the previous one was unclean, `reboot` restarts the machine, the control surface is driven over the connection, the resolver's four answers are told apart, and an orderly `shutdown` is followed by a boot reporting `unclean_boots=0`. `make qemu-operations-closure` runs it alongside the other two. **It is the gate that found this port's `B-126` and `B-127`** -- the image could not carry an SSH key and `reboot` powered the machine off -- which is the argument for a shared gate running on every architecture rather than on the two that were easiest. |
