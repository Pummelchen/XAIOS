...
| QEMU RISC-V `virt` (rv64gc) | Boots the shared kernel to the first-run setup prompt on four harts, with 76 self-tests passing and no errors: Sv48 paging with per-section kernel permissions, SBI console and power, a full trap frame with ecall syscalls and user faults, PLIC interrupts, PCI enumerated through ECAM with base addresses assigned by the kernel because SBI firmware assigns none, two virtio-blk disks, the initial filesystem, IPv6, a Goldfish real-time clock, work spread across harts, and twenty applications including the hosted ISO C99 probes and `xapt`. A UEFI boot medium exists (`scripts/build-riscv64-boot-media.sh`, EDK2 with `-machine virt,acpi=off`) and is unfinished: firmware loads the loader, the loader loads the kernel off the same disk and starts it, and the boot stalls at 45%. Not qualified on hardware: this is one emulated board and nothing else. Run on QEMU 11.1.1 (macOS/arm64). |
