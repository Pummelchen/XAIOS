...
| QEMU RISC-V `virt` (rv64gc) | Boots to 100% on four harts with 81 self-tests and no errors, a login prompt and sshd listening; logging in over SSH returns the machine's real service state. Sv48 paging with per-section kernel permissions, SBI console and power, a full trap frame with ecall syscalls and user faults, PLIC interrupts, PCI through ECAM with base addresses assigned by the kernel because SBI firmware assigns none, both virtio transports, xaiFS mounted at /models, a Goldfish clock, IPv6, the hosted ISO C99 library and `xapt`. Run it with `platform/qemu/run-qemu-riscv64.sh`; the disk layout and `virtio-mmio.force-legacy=false` both matter. A UEFI boot medium exists (`scripts/build-riscv64-boot-media.sh`, EDK2 with `-machine virt,acpi=off`) and is unfinished: firmware loads the loader, the loader loads the kernel off the same disk and starts it, and the boot stalls at 45%. Not qualified on hardware: one emulated board is the whole evidence. QEMU 11.1.1 (macOS/arm64). |
