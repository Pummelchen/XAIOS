...
| V-08 | Console input | `NOT STARTED` | The virtio console driver posts a receive buffer to keep the device from stalling but nothing reads it, so the console is output-only and there is no interactive login on this platform. Implement the receive path and wire it to the console service. |
