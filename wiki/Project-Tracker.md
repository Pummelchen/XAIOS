...
|---:|---|---|---|
| E1 | Disk partitioning, formatting and install tooling | `IN PROGRESS` | More existed than expected: `gpt_write`, `storage_admin_partition_create` and `partition_device_register` are all written, and the control protocol already exposes partition create, delete and plan operations. What was missing is that nothing called them on the boot path -- `/dev/ahci0p0` is a name AHCI gives the whole disk, not a partition. `kmain` now reads the GPT of the disk it booted from and mounts xaibootFS from a partition typed `XAIOS_GPT_TYPE_STATEFS`, before falling back to the separate devices it always used. A GPT disk carrying an EFI System Partition and an xaibootFS partition boots the kernel from one drive with nothing else attached. It does not yet find its state partition, and the reason is specific: when the loader supplies the initial filesystem in memory, that memory device takes `/dev/vblk0` and the physical disk is never enumerated as a block device at all. Enumerating both is the next step, in `kernel/dev/virtio/virtio_blk.c`. |
