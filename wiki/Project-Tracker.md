...
| V-07 | Reachable from the host | `IN PROGRESS` | The NAT attachment carries guest-initiated traffic in both directions but delivers no host-initiated frame, so sshd listens without being reachable. Bridging needs the `com.apple.vm.networking` entitlement that V-03 waits on, but vmnet needs no entitlement, only root, so `tools/vz/vmnet-helper.c` starts a vmnet interface privileged and relays frames to an unprivileged datagram socket the harness attaches. The guest now joins a real vmnet network, takes a DHCP address on it, reaches the host, and appears in the host's ARP table with the right hardware address. Inbound remains broken: the helper writes 120 frames to the socket without error while the guest's device reports four received all boot, ring free and twenty-six million polls behind it, so Virtualization.framework stops reading after the first few datagrams. Sizing SO_RCVBUF at four times SO_SNDBUF as the attachment requires did not change it. Untried: a socketpair rather than bound paths, a lower MTU, driving the socket from the harness itself. |
