...
|---|---|---|---|
| F-01 | Fusion multi-vCPU startup | `BLOCKED (host)` | **The premise was wrong, and the cause is below the guest.** Fusion exposes PSCI, accepts `CPU_ON` for every vCPU and brings them online: `psci=hvc`, `admitted=4 rejected=0`, `online cpus=4/4`. No parking protocol is offered (`parking_version=0`) and no UEFI MP Services handoff is needed. The row previously said otherwise, written before the secondary-CPU bring-up defect was found -- and that defect produced exactly the symptom it described, one CPU of four, on every hypervisor. What blocks four vCPUs here is different: the first lock taken after the kernel enables translation aborts with `esr=0x96000035`, DFSC `0b110101`, on `g_klog_lock` in `.bss`. Two experiments closed this off as a guest defect. First, the kernel was rebuilt with `-march=armv8.1-a` so every atomic is an LSE `LDADD`/`CAS` rather than an `ldxr`/`stxr` pair (verified in the objdump: LL-SC 0, LSE 5); the same abort occurred at the same address, which is consistent -- that DFSC covers atomics as well as exclusives. Second, the stage-1 descriptor for a `.bss` word was printed rather than inferred: `0x60000090594703`, which decodes as valid page, `AttrIndx=0` (MAIR[0] = `0xff`, Normal Write-Back), `AP=00` (read/write at EL1), `SH=11` (Inner Shareable), `AF=1`, PXN/UXN set. `SCTLR_EL1` sets M, C and I. Those are exactly the conditions under which atomics are architecturally supported, so XAIOS's mapping is correct and the refusal comes from Fusion's stage-2 mapping or CPU model, which the guest cannot reach or observe. Nothing further is actionable from inside XAIOS; reopen if VMware documents a guest-side requirement. `numvcpus` stays at 1 so the profile passes; raise it to reproduce. |
...
| C-02 | network_stack.c state escapes its module | `TESTING` | The obstacle to finer locking was never the lock: it was that tables handed out their rows. The socket map returned interior pointers across the syscall boundary, where callers dereferenced them after the guard had been released -- a live race once secondary CPUs ran -- and now returns a copy. The listener registry leaked rows to seven functions inside the module, all of which now hold its own guard, so bind and close no longer serialise against the whole stack. **No exported function returns a pointer into module state any more**, and the interior lookups that remain -- flows, queues, packet descriptors -- are reached only through guarded entry points. Splitting those tables further is performance work and waits on C-03, which needs a machine that can measure rather than more code. |
