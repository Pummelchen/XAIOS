...
budget now (B-138) and the gate gives it the same 300 seconds it gives the boot, which
is the fix and not a coincidence: the gate reports a completed handshake on this Mac
with `wait_seconds=300` in the peer's own line, and the `macos-14` job -- the workflow's
only macOS job -- passes on the commit that carries it.
...
| `B-138` | The host peer waited twenty seconds for a guest that takes longer to boot | `75d2b26` Give the host peer the gate's budget to wait for a booting guest |
