...
|---|---|---|---|---|
| B-43 | A session is accepted, never authenticated, and stalls ~19 s | VMware Fusion ARM64 | `TESTING` | Round 422 of 1658: `sftp exited 255 ('Connection closed')`, 18.22 s where neighbouring rounds are 0.73 s apart. The guest's console is specific this time. `net_accept connfd=851` is followed by **nothing at all** -- no authorized-key parse, no session -- while `connfd=852`, accepted *after* it, authenticates and is served to completion. So the server was running and serving a later connection while an earlier one sat untouched. No refusal marker fired, which rules out all three pre-serve paths: it was not turned away, it was accepted and then not serviced. It is not `B-38`: that fix was in this build, and the soak's payload -- `(i*31+7)&0xFF`, consecutive bytes differing by 31 -- provably cannot contain `ESC[`. The accept/close tally was clean, so it was closed eventually. **To close:** find what leaves an accepted connection unserviced while later ones proceed, and what the 18 s corresponds to; the connect-timeout path logs only to the audit file on the durable volume, which no soak reads. |
