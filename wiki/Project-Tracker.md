...
|---|---|---|---|---|
| B-38 | An SFTP session stalls when the file contains the alternate-screen sequence | all four | `TESTING` | **Root cause found and fixed; attribution to the original occurrence is not claimed.** `ssh_channel_send_data` ran every channel's outgoing bytes through the screen framework's session filter, which watches for `ESC [ ? 1 0 4 9 h` and then paints rather than forwards. SFTP carries arbitrary binary, so a file containing those eight bytes has its payload eaten mid-transfer, the length-prefixed framing desynchronises, and the client waits forever for a response the server believes it sent -- a quiet authenticated connection with no error and nothing to close, which is B-38's signature exactly. `direct-tcpip` forwards and agent channels had the same exposure. Fixed by gating on what the channel is, since no byte test can work on arbitrary payload, and held by `make qemu-sftp-binary-passthrough-gate` -- red without the fix, and its control payload passes in the same run, so the harness is proven sound. **Two claims this row made are withdrawn:** the socket is not leaked, it is reclaimed at ~300 s by `SSHD_TIMEOUT_IDLE`, measured twice; and the missing `net_close` proved nothing, because the soak captures ~1.4 s of console per round and a 300 s reclaim lands some 400 rounds later. **Not proven to be the 1569th round:** that soak's payload is `(i*31+7)&0xFF`, whose consecutive bytes differ by 31, so it cannot contain `ESC[` at all. **To close:** a soak that keeps a console window longer than 300 s, or simply tallies accepted-minus-closed at end of run, so the next occurrence is attributable. |
...
| B-42 | The DNSSEC self-test fixture and its anchors ship in release images | all four | `TESTING` | `dns_self_test()` is called unconditionally at `kernel/core/kmain.c:1060`, so the committed signed chain in `kernel/net/dns_selftest_chain.h` and the test anchor install compile into every build, release included. B-33's fixture *zone* is correctly behind `XAIOS_BOOT_TEST_APPS` and a release resolver has no fixture names and no branch testing for them; the data underneath does not have that guard. Noticed while closing B-33 and not extended there. **To close:** decide whether a release image should carry a test chain and anchors at all -- if not, guard the data and the self-test call together, and check the image size and the `production-source-audit` both agree afterwards. |
| B-40 | `send_all` can freeze the whole server for as long as a peer trickles | all four | `TESTING` | `userspace/sshd/ssh_protocol.c` bounds a transmit stall at 10 s, but `stalled_since` resets on **any** byte sent, so a peer that makes the smallest progress inside each window restarts the clock indefinitely. sshd is single-threaded, so that is not one slow session -- it is every session, the listener included. Found while tracing B-38's candidates. **To close:** bound the whole transmit rather than the gap between bytes, and gate it with a peer that reads one byte per interval. |
| B-41 | A failing channel is logged, never closed, and starves the ones after it | all four | `TESTING` | `userspace/sshd/sshd.c` logs a failure from `ssh_channel_tick()` and then `return`s, so the channel stays open and every channel later in the list is not serviced on that tick. It cannot stall an SFTP transfer, which `tick` does not touch, but it can stall a forward or an interactive session. Found while tracing B-38's candidates. **To close:** close the failing channel and continue the loop, with a gate that holds a second channel open behind a failing one. |
