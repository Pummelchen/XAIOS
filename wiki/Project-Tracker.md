...
| B-05 | One fixed kernel link address blocks a 1 GiB profile | all four | `OPEN` | The kernel is linked at a fixed `0x90000000`, and the hypervisors do not agree on where usable memory starts: QEMU `0x40000000`, Virtualization.framework `0x70000000`, VMware Fusion `0x80000000` (measured, not assumed -- a probe over the UEFI memory map on each). A QEMU guest given one gibibyte owns `[0x40000000, 0x80000000)`, which ends exactly where Fusion's memory begins, so no single fixed address can sit inside both. One gibibyte uniformly therefore needs a relocatable kernel -- position-independent, or relocated by the loader -- not a smaller number in a profile. Two gibibytes is the smallest figure that boots on all four today. Splitting the link address per platform would work and is rejected: it would give each hypervisor its own kernel image, which is what `docs/PLATFORM-NEUTRALITY.md` exists to prevent. |
| B-06 | Virtualization.framework boots to nothing below ~3.5 GiB | Virtualization.framework | `OPEN` | Bisected: 3584 MiB boots and reaches every marker, 3328 MiB and below produce no guest serial output at all -- the harness starts and nothing follows, with no error anywhere to read. The RAM arithmetic does not explain it. Usable memory there begins at `0x70000000`, so three gibibytes covers the kernel's `[0x90000000, 0x90625000)` under any placement model tried, and neither a fixed base nor a top-anchored one predicts a floor at this value. Cause still unknown. `run-vz.sh` now refuses the range with an explanation rather than handing back a black screen, overridable with `XAIOS_VZ_ALLOW_LOW_MEMORY=1` for whoever picks the investigation up. That converts the worst property of this bug -- silence -- into a message, but it is containment, not a fix. |
| B-04 | Fusion DHCP intermittently gets no offer | VMware Fusion ARM64 | `TESTING` | **Fixed.** The client used one compile-time transaction id, `0x5841494f`, for every DHCP exchange of every boot of every guest. RFC 2131 requires a randomly chosen xid per transaction, and a server that has already completed one carrying that xid for that MAC is entitled to treat the next as a retransmission and stay silent -- which is exactly what this looked like: link up, `STATUS.LU` set, DISCOVER sent, no OFFER, three times over. Whether it failed depended on whether the server still remembered, which is why it was intermittent and why the gate's reboot made it worse. The xid is now drawn per transaction from the entropy source, with the timer as a fallback and the MAC mixed in. Retransmission also backs off and carries jitter rather than retrying at a fixed one-second interval, and the boot budget went from six seconds to fifteen, which was barely two attempts once backoff is honoured. Eight consecutive Fusion smoke runs with zero offer failures, against roughly one boot in three before. |
