...
| Boots at 1, 2 and 4 GiB | yes | yes | 1 and 2 GiB; `B-12` at 4 | yes |
...
| B-12 | Fusion faults on a device above the identity map at 4 GiB | VMware Fusion ARM64 | `OPEN` | At 4 GiB the guest takes a level-1 translation fault on `far=0xff0000000` -- 63.75 GiB, where this firmware places a PCI window once RAM is large enough to push it up there. The kernel identity-maps physical memory up to the RAM extent and expects device windows to fall inside it, which holds until firmware puts one past the end. Predates the relocation work and is unrelated to it: the same fault appeared before. 1 and 2 GiB pass, and the fix is to map discovered device windows rather than assume the identity map covers them. |
