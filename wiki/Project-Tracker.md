...
kits). Two defects came out of that work rather than out of reading: the Fusion
chainloader step hung in a container with a bind mount of the tree, and **Fusion
panicked on boot** because `klog`'s drop counter was read before the kernel was
on its own page tables -- a bisect over the 333 commits since build 6 named the
...
failing step. In order: the host WebTransport tests linked only the *verify*
half of the signature primitives (the port began signing with a loaded key and
the link failed on five symbols from that day); behind that, the QUIC packet
suite's split module used counter names the split had renamed, which nothing had
compiled because the link failed first; behind that, the interop peer compiled
the vendored trees with this repository's `-Werror` and a hard-coded `llvm-ar`,
both of which fail on the runner and neither of which is XAIOS code; and behind
that, `qemu-quic-handshake-gate` called a function that does not exist while
reporting its own timeout. The host tests (1,572 checks), the sanitizer run and
...
What was current when build 6 was cut, against `7df4fc8`:
`make release-check` passed, which requires `docs-check`, a
...
kits) also passed on build 6's released files.
...
| `B-137` | The booted-guest WebTransport handshake gate fails on the CI runner | `IN PROGRESS` | The guest boots and `/bin/wtqtest` runs, and then reports `packets=0 bytes=0 received=again` for 20,000 rounds and `WT-HANDSHAKE-FAILED timed out`, after which the boot harness fails its own assertion that the application exits 0 -- which is the cyan screen the gate reports as a panic marker, a consequence and not a second defect. **The guest's network is up when it happens:** the run logs a DHCP lease (`ip=0a00020f gw=0a000202`) and a validated host ARP reply from `10.0.2.2`, and the peer binds and listens before the guest starts. The same gate passes on this host -- `make qemu-quic-handshake-gate` writes its report and prints `the guest completed a WebTransport handshake`, exit 0 -- so what differs is the runner, not the image. The gate's failure path was itself broken until `503d506`: it called a function defined nowhere and died of a `NameError` instead of reading the peer's log, which is why no failing run has ever said whether the peer saw a packet. The transcript now settles it: `WT-PEER-BOUND port=4433`, then `WT-PEER-TIMEOUT no Initial arrived` -- the peer was listening and the guest's first packet never reached the host. Nothing about that is XAIOS: the guest's own stack is up, and the same gate hands a completed handshake on this Mac. The runner's user-mode network does not deliver guest-to-host UDP, which is the one path this gate uses that no other CI gate depends on -- SSH and the rest arrive through `hostfwd`. **What is being done about it:** the gate now has its own CI job on an Apple Silicon macOS runner, where it passes, instead of a step in the Linux boot job. It is the repository's only macOS job, the platform being the reason it needs one. The row stays open until that job has been seen green: an intent to fix a red check is not a fix. The job keeps the peer transcript, so a future failure still says whether the packet moved. **Exit criterion:** the gate passes on the runner, or the reason it cannot is named and the gate reports not-checked rather than failed. |
...
**What this project has to run on, as of 2026-09-18: Macs and one Intel VPS.**
QEMU runs on all of them for all three architectures, and VMware Fusion is
available on `node2` only. Those are the environments a release claim rests on,
and every row of the QEMU and Fusion columns above is gated on this machine.
...
host's current state.** It was collected when the guest came up; it does not
today, and the difference is the host (macOS 27.0) rather than the tree, because
the released build 6 image is equally silent. Anything read from that column
should be read as the platform's shape, not as a measurement taken now.
...
**No model or inference work is being started now.** At the maintainer's
direction on 2026-09-18 the rows below are a boundary, not a queue: they say what
would have to be true before the work could be claimed, and none of it is
underway. The engine's own tests and the host model-v2 suite still run in CI;
nothing new is being added to them.
...
**On 2026-09-18 the guest does not come up at all here, and the host is the
variable.** The harness starts -- `xaios-vz: started cpus=4 memory=2048MiB` --
and the firmware runs: a screenshot of the window shows `Booting 'XAIOS'` and
the path it loads, and then nothing. No kernel line, no framebuffer console, no
serial output, for 420 seconds, with a fresh EFI variable store as well as the
existing one. **The released build 6 image, which passed this row when build 6
was cut, is equally silent**, and so is a boot disk built from the current tree,
so this is not a change in the tree. This host is macOS 27.0 (26A428).
`make vz-gate`, `make vz-stress-gate` and the release gate's VZ row are
therefore **not checked** here, and the local-gate record carries them as
failures that describe the host rather than the software.
...
| `B-136` | Source files ran to thousands of lines and the 500-line rule was not enforced | `e78137d` Split the last ten files; the size baseline is empty |
