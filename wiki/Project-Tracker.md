...
|---|---|---|---|---|
| B-28 | An SSH session is refused once in several hundred under sustained load | VMware Fusion ARM64 | `TESTING` | **Diagnosed, and it is arithmetic rather than a mystery.** sshd rate-limits accepts per client address at `SSHD_CONNECTION_RATE_LIMIT` per `SSHD_CONNECTION_RATE_WINDOW` -- 120 per minute. A soak round opens two connections, the transfer and the status probe after it, so round 61 carries connection 121 and is closed before a byte of SSH is spoken. `make qemu-ssh-connection-rate-gate` reproduces it deterministically under QEMU: 120 served, the 121st closed, a control of 30 clean. It was invisible for two years because the rate-limit path logged only to the audit file on the durable volume, the capacity path announced itself once per boot, and the slot-exhaustion path said nothing at all; all three now name their reason on the console with a per-reason counter. Authentication credits the connection back to the window, since the limit exists to bound a peer that has proved nothing. **To close:** the credit's own gate case needs a quiet host -- a real SSH session against a TCG guest is slow enough that the case has to fit inside the window to mean anything. |
