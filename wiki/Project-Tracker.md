...
| B-44 | sshd is the machine's network thread, and nothing says so | all four | `TESTING` | `network_poll_tick()` drains the NIC ring and runs the whole TCP state machine -- ACKs, retransmits, flow expiry -- and has no timer, no interrupt handler and no kernel thread behind it. Its callers are the network syscalls, `xaios_wait_events`, and one boot-time NTP loop; from userspace only `sshd` and `xtop` call `wait_events`. So the guest's networking runs only while sshd is in its loop, and any pause anywhere in that loop -- a slow log write, a filesystem stall, anything -- is a total outage: no timeout fires, nothing is refused, nothing is closed, and from outside it is indistinguishable from the machine having gone away. `B-43` is the first sighting that is probably this. **To close:** decide whether the poll belongs on a timer or a thread of its own rather than borrowing whichever process happens to call a syscall; until then, say it in the architecture documentation, because a reader would not guess it. |
| B-45 | Every 32-byte audit append rewrites the whole log file | all four | `TESTING` | `xaiboot_fs_write_fd_locked` reads the entire file and writes it entirely back for each append, so `ssh_log` costs a whole-file read-modify-write per record -- **measured at 7.1 per connection**, up to 7 KiB and 12 blocks to the durable volume, growing until the log rotates. `load_authorized_keys` re-reads `/etc/xaios_authorized_keys` from the filesystem on every publickey attempt, twice per connection. Both sit inside sshd's loop, which by `B-44` is the machine's network thread, and one of them is issued immediately after the `net_accept` klog -- exactly the gap in B-43's console. Not proven to be B-43 and not claimed as it. **To close:** append without rewriting, and cache the authorized keys against the file's generation. |
| B-46 | A full-size SSH packet would be refused rather than short-written | all four | `TESTING` | `SSH_MAX_PACKET_SIZE` is 35000; the syscall's `XAIOS_SYSCALL_NETWORK_IO_MAX_BYTES` is 16384. A send or recv at full size comes back `net-send-denied` instead of a short write. Not reachable today because `SSH_CHANNEL_MAX_PACKET` is 10240, so it is a latent cliff rather than a live defect -- and the kind that surfaces the day someone raises the channel maximum for a good reason. **To close:** make the two agree, or have the protocol layer chunk to the syscall's limit and prove it with a packet larger than 16384. |
| B-47 | A socket map that is full accepts the connection and drops it silently | all four | `TESTING` | `network_stack_map_socket_unlocked` does nothing when the map is full: the accept succeeds, the descriptor is returned, `syscall: net_accept` is logged, and the socket has no flow. That is an exact mechanism for "accepted and then never progressed" -- the shape `B-43` was originally read as. It is not that occurrence: the map is sized to the flow table so it cannot fill before flows do, and the run's accept/close tally was clean. It should still fail loudly rather than quietly. **To close:** refuse the accept, or log the exhaustion, and gate it by filling the map. |
