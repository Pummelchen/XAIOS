...
| C-01 | Shared kernel state under genuine parallelism | `OPEN` | Until secondaries actually ran, kernel subsystems had one CPU between them and unsynchronised shared state could not be observed to fail. That assumption is gone. A sweep counts several hundred non-atomic updates to shared globals outside self-tests, and the protection is uneven: `mutable_fs.c` has 24 lock sites and keeps its counters inside them, `network_stack.c` has 3 against 118 counter updates, `user/service.c` none against 26. Syscalls run on whichever CPU the calling thread occupies, so all of it is reachable now. Two instances are already fixed rather than theorised -- a `net_close` race that halted the kernel from userspace, and 13 process counters left plain beside atomic siblings -- which is what makes the rest worth reading properly. This wants a deliberate pass per subsystem deciding what is genuinely concurrent and what is serialised by construction, not a blanket conversion: most of these counters may be fine, and changing them all would be a large unreviewed change to code that currently works. |
