...
| B-15 | An intermittent fatal assertion on VMware Fusion | VMware Fusion ARM64 | `OPEN` | One `make local-gates` run reported `System halted`, `assertion failed` and a cyan screen from `vmware-fusion-smoke`; the four runs before and after it passed. Which assertion is unknown, because Fusion writes its guest console to one file in the VM bundle and every later gate in the same run overwrites it -- by the time the run finished there was nothing left to read but a stack trace of addresses. `local-gates` now keeps each gate's console beside its output, so the next occurrence is diagnosable. Not reproduced in five subsequent runs. |
| B-14 | The x86_64 guest writes to the medium it booted from | QEMU x86_64 | `OPEN` | Its virtio-blk self-test exercises write, error and reset against device zero, which is the boot medium, so booting an image modifies it. Noticed when a release artifact's checksum moved between being gated and being published. The AArch64 runner attaches the boot drive read-only and the x86_64 one cannot, because that self-test fails against a read-only device. It matters beyond checksums: a machine booting real read-only media -- a CD, or a write-protected stick -- would fail that self-test rather than skip it. The gate now boots a copy so artifacts are never mutated; the underlying behaviour is unchanged and wants the self-test pointed at a device that is not the boot medium. |
