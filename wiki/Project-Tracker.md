...
| B-52 | A machine that has run long enough stops booting | all four | `FIXED` | `allocate_extents` in `kernel/fs/xaiboot_fs.c` was first-fit and gave up once a file needed more than `XBFS_V6_MAX_EXTENTS` (16) extents. The low blocks of a volume are the most broken up, so it collected sixteen short runs out of the rubble at the bottom and never reached the long runs above them; `clone_extents` turned the refusal into `XAIOS_ERR_IO` and `commit_snapshot` failed on a filesystem `fsck` calls sound. **The consequence was a machine that would not boot.** `kmain.c:1037` runs `update_self_test()` on every boot with a writable persistent filesystem, staging writes a snapshot, and `update.c:752` asserts that staging worked -- so a volume aged this far halts the machine in the cyan screen. Not a failed update; a failed boot, on a machine that worked yesterday. This repository's own development Mac had reached that state after 64 boots. **Fixed by taking the longest runs first**, which is not merely better than address order but the best any policy can do against this constraint: the question is whether some sixteen runs can cover the file, and if the sixteen longest cannot, no sixteen can. One scan, ended early by the first run long enough to hold the whole file, so the case that always worked costs what it always did. No on-disk structure changes -- extents are still extents, only the choice of blocks differs, and existing volumes read unchanged. Allocation is also claimed only once the whole placement is known, so a refusal no longer moves the allocation and free counters. **Evidence.** `tests/storage/test_xaiboot_fs_fragmentation.c` ages a v5 volume of the same 8192 data sectors the durable volume uses until first-fit cannot place a 32 KiB file, then requires the write, the read-back, the snapshot and a remount; against the old allocator it fails at the write. And on this machine's actual `build/xaios-persistent.img`, mounted read-only in a hosted harness: old allocator `commit = -4`, new allocator `commit = 0`, same bytes, `fsck` clean, v5. **Still open underneath it:** sixteen extents is a real ceiling and a volume can still be aged past what any sixteen runs cover. Compaction, or a wider extent table, which is a format change. |
...
| B-54 | The release-image gate passed a row that booted a kernel from the previous build | repository | `FIXED` | Its whole claim is that the file a release ships boots. On the Virtualization.framework row it was not true and nothing noticed: `boot_vz` handed the harness `vz-system.img` and `vz-system2.img`, the loader prefers a verified A/B slot over the kernel on the medium, and the guest printed `XAIOS Build 5 kernel starting` while the gate was testing build 6's image. The two QEMU rows and Fusion all logged `system-slot: unavailable` and ran the medium's kernel; this one silently did not, and had been reporting success that way. **The blind spot is the more useful half.** The marker was `XAIOS Build \d+ kernel starting` -- any build number matched, so the one check that could have caught it said nothing. A gate that accepts a kernel from another build cannot establish that this file boots. The QEMU path had set `XAIOS_SYSTEM_VOLUME_IMAGE=none` for exactly this reason, with the reason written above it; the VZ path was never given the same treatment. **Fixed:** the two system volumes are dropped from the VZ harness call (they are last in the list and the harness attaches disks in order, so nothing shifts), and the marker now names the current build. **Evidence:** the old marker matches `XAIOS Build 5 kernel starting` and the new one does not; after the fix all five rows report build 6 and `system-slot: unavailable`. |
