...
|---|---|---|---|
| F-01 | Fusion multi-vCPU startup | `IN PROGRESS` | **The premise was wrong.** Fusion exposes PSCI, accepts `CPU_ON` for every vCPU and brings them online: `psci=hvc`, `admitted=4 rejected=0`, `online cpus=4/4`. No parking protocol is offered (`parking_version=0`) and no UEFI MP Services handoff is needed. The row previously said otherwise, written before the secondary-CPU bring-up defect was found this morning -- and that defect produced exactly the symptom it described, one CPU of four, on every hypervisor. What actually blocks four vCPUs here is different: the first lock taken after the kernel enables translation aborts with `esr=0x96000035`, an unsupported exclusive, on `g_klog_lock` in `.bss`. That address is inside `__bss_start..__bss_end`, gets Normal Inner-Shareable attributes, and translates cleanly, so exclusives on it should be legal. Fusion alone rejects them. `numvcpus` stays at 1 so the profile passes; raise it to reproduce. |
