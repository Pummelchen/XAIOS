...
| B-03 | vmnet-helper spins a core while idle | `OPEN` | The privileged relay polls its descriptors rather than blocking on them, so it holds a core busy whether or not a guest is attached. It was at 88% of one core with no VM running, and it corrupted the C-03 measurement by doing so. `poll` with no timeout on both descriptors is what this wants; the loop already knows which ones to watch. |
