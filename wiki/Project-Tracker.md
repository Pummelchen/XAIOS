...
|---:|---|---|---|
| D1 | USB image, every architecture | `PARTIAL` | `xaios_b1.iso` is a hybrid ISO 9660 and GPT disk carrying both an AArch64 and an x86-64 kernel, and firmware picks its own. The GPT and its EFI System Partition are what a stick boots from, so `dd` should produce one. Still never written to a stick and booted: nothing physical has run this, and no gate here can change that. What did change is that the alternative now exists -- a machine can be installed onto from a running XAIOS rather than only imaged from a host. |
| D2 | Network boot for blank machines | `NOT STARTED` | Nothing exists -- no PXE, no HTTP boot, no netboot path of any kind. Its dependency is now met: network boot is only worth having if the booted system can install itself onto the machine's disk, and as of E1 it can. |
