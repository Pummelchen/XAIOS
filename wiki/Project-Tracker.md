...
| Boots the unified image | `B-13` | yes | yes | yes |
...
| B-13 | x86_64 trips a virtio-block self-test when booted from the unified image | QEMU x86_64 | `OPEN` | Booting `build/xaios.iso` on x86_64 reaches the kernel and loads its own initial filesystem (`source=uefi-initfs`), then asserts in `virtio_block_interrupt_canary_arm`. The same kernel and the same initfs boot cleanly from the per-architecture image, so this is about how devices enumerate when the boot medium is the ISO rather than a plain FAT image -- most likely which virtio-blk index the canary arms against. The other three environments boot the unified image to a login prompt. |
