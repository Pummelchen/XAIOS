...
| B-04 | Fusion DHCP intermittently gets no offer | VMware Fusion ARM64 | `OPEN` | Roughly one boot in three ends `network: DHCP offer not received status=-4` after three retries, and the guest comes up without IPv4. Seen at one vCPU before the F-01 fix and at four after it, so it is not related to CPU count. The guest sends and retries; whether the host's NAT service answers is not yet established, so this is not yet attributed to either side. It matters more than its frequency suggests: a lease is a precondition for SSH, so a flaky lease reads as a flaky OS. |
