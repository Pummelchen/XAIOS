...
| C-02 | network_stack.c cannot be split until its state is encapsulated | `IN PROGRESS` | 5,168 lines, and measurement says the length is a symptom. Cutting after the declarations moves 11 symbols across the boundary; cutting before the TCP data plane moves 88; extracting the self-test moves 50. The cause is a file-scope state block -- every counter, both flow tables, the listener registry -- that every layer touches, so any split relocates the coupling into a shared header without reducing it, and an 88-symbol internal header is a worse interface than one long file. The real work is putting that state behind accessors first, one group at a time with gates between, after which the seams become real. Doing it alongside other work is how a network stack acquires a subtle bug, so it wants its own pass. The file now carries a measured map of its own layout for navigation. Two tables are done and the pattern is clear: the obstacle is not the lock but that rows escape. The listener registry handed interior pointers to seven functions inside the module, all of which now hold its own guard; the socket map handed them to `syscall.c`, which dereferenced them after the guard had been released -- a live race since secondary CPUs began running -- and now returns a copy. Every remaining table needs the same treatment before its guard can be split. |
| C-03 | The socket path does not scale, and this machine cannot measure it | `BLOCKED` | The original finding stands: socket bind/close cost 97,642 ns at one thread and 437,018 at four with wall time flat from four threads to eight, which is a serialised section. The listener registry now has its own guard so those operations no longer serialise against the whole stack. **Whether that helped cannot be established here.** Two runs of identical code, minutes apart, produced 131,232 ns and 425,894 ns at four threads -- 2.7x apart at eight -- because host load moved between them. The noise exceeds any effect worth measuring, so no A/B on this machine can settle it, and one taken anyway would be a coin toss dressed as evidence. What this needs is a quiet machine: no other workload, no other virtual machines, several runs per configuration, and the spread reported rather than a single number. Until then the guard split is justified by correctness -- rows no longer escape the tables -- and not by speed. |
