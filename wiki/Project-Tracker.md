...
| B-55 | Sixteen extents is a real ceiling, and a volume can still be aged past it | all four | `TESTING` | What `B-52` fixed was the *policy*: blocks are taken longest run first, which is the best any placement can do against a sixteen-extent limit, so a volume has to be far more broken up before it fails. The limit itself is unchanged. `XBFS_V6_MAX_EXTENTS` is 16 (`kernel/fs/xaiboot_fs.c:86`), and a file whose blocks cannot be covered by the sixteen longest free runs is still refused -- which still means `commit_snapshot` fails and still means the boot-time update self-test halts the machine. No volume here is near it now; the one that failed had 1550 of 8192 blocks used and now snapshots cleanly. **To close:** either compact on allocation failure, or widen the extent table, which is an on-disk format change and needs the v5/v6 conversion path to carry it. Either way the measurement comes first: `tests/storage/test_xaiboot_fs_fragmentation.c` ages a volume on purpose and is the place to find where the new policy actually gives out. |
