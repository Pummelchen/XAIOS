...
| B-06 | Virtualization.framework will not boot below 4 GiB | Virtualization.framework | `OPEN` | 4096 MiB boots and the guest reaches every marker; 3072 MiB and 2048 MiB produce no guest serial output at all -- the harness starts, and nothing follows. The RAM arithmetic does not explain it: usable memory there starts at `0x70000000`, so three gibibytes covers the kernel's `[0x90000000, 0x90625000)` with room to spare. Something else has a floor. It matters beyond the memory figure, because a guest that silently produces nothing is the hardest kind of failure to diagnose in front of an audience, and nothing currently detects it -- `vz-gate` fixes 4096 MiB, so the floor has never been crossed by a gate. |
