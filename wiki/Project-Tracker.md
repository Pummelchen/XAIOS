...
| C-03 | The socket path does not scale across cores | `TESTING` | Measured on eight real cores with `/bin/perfbench`: socket bind/close cost 97,642 ns at one thread and 437,018 at four, with wall time flat from four threads to eight -- the signature of a serialised section, which was the coarse network guard from C-01. The listener registry, which is all bind and close touch, now has its own guard, so those operations no longer serialise against the whole stack. **Whether that helped is unmeasured.** The A/B on this machine was incoherent -- 989 us at one thread against 74 us at four in the same run -- because the host was at load average 37 with an unrelated application running and `vmnet-helper` spinning a core while idle. Re-measure on a quiet machine before claiming anything; the correctness gates pass either way. |
