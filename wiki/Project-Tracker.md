...
host peer through QEMU's user-mode network, which GitHub's Linux runners do not use
to deliver guest-to-host UDP: with the peer's transcript finally printed, the run
showed `WT-PEER-BOUND port=4433` and then `WT-PEER-TIMEOUT no Initial arrived` -- no
packet at all -- while the same boot logged a DHCP lease and a validated ARP reply
from the gateway. The gate now has its own `macos-14` job, the workflow's only macOS
job, and it reports `the guest completed a WebTransport handshake` there.
...
| `B-137` | The booted-guest WebTransport handshake gate failed on the Linux CI runner | `a7c080a` Run the guest handshake gate on a macOS CI runner |
