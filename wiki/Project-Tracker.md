...
|---:|---|---|---|
| E1 | Disk partitioning, formatting and install tooling | `IN PROGRESS` | More existed than expected and none of it was connected: `gpt_write`, `storage_admin_partition_create` and `partition_device_register` are written, the control protocol exposes partition operations, and nothing called any of it on the boot path -- `/dev/ahci0p0` is the name AHCI gives a whole disk, not a partition. `kmain` now scans every registered block device for a GPT and mounts xaibootFS from a partition typed `XAIOS_GPT_TYPE_STATEFS`. A GPT disk with an EFI System Partition and a state partition boots the kernel from one drive with nothing else attached. It still does not find its state partition, and the chain is now understood: when the loader supplies the initial filesystem in memory, `virtio_block_init` returns before probing the transport, so no physical disk is registered at all; calling `virtio_block_open_slot` from the boot path does not register one either, over PCI or MMIO, and it does not say why. The next step is inside that function rather than in `kmain`. Scanning all devices instead of naming one is kept regardless: naming `/dev/vblk0` reads a GPT out of an initfs image on exactly the configuration this feature is for. |
