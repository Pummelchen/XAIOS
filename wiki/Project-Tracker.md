...
| B-36 | A malformed argument is reported as a DNSSEC failure | all four | `TESTING` | `kernel/runtime/operations.c` maps `XAIOS_ERR_INVALID` to `dnssec-unverified`, and that status means both "bad arguments" and "the chain did not validate". `nslookup example.com extra-argument`, `nslookup` with no name and `nslookup -6` all print `dnssec-unverified`. `tests/scripts/qemu-operations-closure.py` accepts that string as a valid answer, so a malformed invocation reads there as a fail-closed resolver. **To close:** separate the two statuses, and make the gate reject the argument error rather than accept it. |
...
| B-28 | An SSH session is refused once in several hundred under sustained load | VMware Fusion ARM64 | `TESTING` | **Diagnosed, and it is arithmetic rather than a mystery.** sshd rate-limits accepts per client address at `SSHD_CONNECTION_RATE_LIMIT` per `SSHD_CONNECTION_RATE_WINDOW` -- 120 per minute. A soak round opens two connections, the transfer and the status probe after it, so round 61 carries connection 121 and is closed before a byte of SSH is spoken. `make qemu-ssh-connection-rate-gate` reproduces it deterministically under QEMU: 120 served, the 121st closed, a control of 30 clean. It was invisible for two years because the rate-limit path logged only to the audit file on the durable volume, the capacity path announced itself once per boot, and the slot-exhaustion path said nothing at all; all three now name their reason on the console with a per-reason counter. Authentication credits the connection back to the window, since the limit exists to bound a peer that has proved nothing. The credit's gate case is proven in both directions on a quiet host: with the credit both sessions are served, with it disabled the second is refused, each run inside the window. **To close:** one Fusion soak. The mechanism and the fix are established under QEMU; what has not been re-run is the 586-round soak that reported the original symptom. |
