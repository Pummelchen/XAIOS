...
|---|---|---|---|---|
| B-28 | An SSH session is refused once in several hundred under sustained load | VMware Fusion ARM64 | `TESTING` | Reproduced and measured for the first time: one refusal (`sftp exited 255`) at round 61 of 586 on a single boot under `make vmware-fusion-load-soak`, with the guest answering between every round. Not the session table (B-25's defect, now LRU-evicted) and not a leak -- free memory moved by zero pages across the run. **To close:** reproduce it a second time with the guest-side log of the refusing connection, then name the path. |
| B-31 | `qemu-libc-gate` boots a RISC-V image its dependency never builds | all four | `TESTING` | `image-libc-test` builds only the AArch64 and x86-64 images; the gate then iterates all three architectures, so the RISC-V leg boots whatever `build/` already holds -- possibly another commit or the release configuration. It has been green throughout, which is the point: a leg testing an unrelated artefact passes as convincingly as one testing the right one. **To close:** build the RISC-V image in the dependency or in the gate, then confirm the leg still passes with no other RISC-V build present. |
| B-32 | The CI permissions check cannot see a job-level override | repository | `TESTING` | `check-code-scanning-contract.py` matches the workflow's top-level `permissions:` block and stops at `jobs:`, so no per-job override is visible to it -- a job could grant itself anything and the check would still pass. Found because `publish-wiki` legitimately holds `contents: write` while the documentation claimed read-only. **To close:** parse the workflow, enumerate each job's effective permissions, and require every elevation to be named. |
| B-30 | `operations-closure` misses an unclean boot on a loaded machine | all four | `TESTING` | Seen once: `missing ['unclean_boots=1']` from `qemu-qualification-readiness`. Not inherited state -- the test unlinks its own volume first -- and not a regression, since the same command passed standalone and in the aggregate on the same tree. The failing run took 72.9 s against 241.9 s for the passing one, consistent with the lifecycle record not yet being durable when the kill landed. That is a hypothesis. **To close:** have the first boot wait for the lifecycle record rather than for SSH, then re-run under the same load. |
| B-02 | Thread join failed under load, twice | all four | `TESTING` | A defect with exactly this signature is fixed -- `user_thread_worker` cleared to the kernel unconditionally, so a nested run inside `xaios_thread_join` left the outer syscall with no current process and the wrong address space. It now saves and restores the CPU's process, address space and thread slot. Whether that produced *these* two sightings is inference: neither was reproduced deliberately. **To close:** a loaded reproduction, or enough loaded boots to call it gone. |
...
| E4 | Multiqueue and RSS networking | `IN PROGRESS` | Per-queue driver state, every advertised pair allocated and polled round-robin so a busy pair cannot starve the others, and `VIRTIO_NET_CTRL_MQ_VQ_PAIRS_SET` sent only after the buffers exist -- a device told to use four pairs delivers on four immediately. **Remaining:** RSS itself, and steering evidence on a device that offers more pairs than this one. |
...
|---|---|---|---|
| F-03 | Fusion network feature qualification | `TESTING` | The IPv6 half is unblocked: `e1000e` programmed its receive control with an empty multicast table, so every multicast frame was discarded in the NIC and no router advertisement could reach the stack. IPv4 never noticed, because DHCP is broadcast. Fixed, and `make vmware-fusion-network-gate` now passes on the LAN with a real lease, a global SLAAC address, ICMPv6, SSH on both families and an SFTP round trip over IPv6. **Remaining:** outbound SSH/SCP, local DNSSEC interoperability and forwarding; loss and reorder are not claimed, because the LAN is not a controlled link. |
| F-04 | Fusion snapshot and sustained-load qualification | `IN PROGRESS` | `make vmware-fusion-snapshot-gate` states the guarantee as three properties and requires each: a snapshot is a point in time (both halves -- data before it survives a revert, data after it does not), a revert lands on a filesystem the guest trusts, and a suspend is not a power cut. Nine checks pass, taken with the guest powered off, which is a deliberate choice. **Remaining:** the sustained-load half. `make vmware-fusion-load-soak` is the harness and reported B-28. |
