...
| `B-137` | The booted-guest WebTransport handshake gate fails on the CI runner | `IN PROGRESS` | The guest boots and `/bin/wtqtest` runs, and then reports `packets=0 bytes=0 received=again` for 20,000 rounds and `WT-HANDSHAKE-FAILED timed out`. The same gate passes on this host -- `make qemu-quic-handshake-gate` writes its report and prints `the guest completed a WebTransport handshake`, exit 0 -- so what differs is the runner, not the image. The gate's failure path was itself broken until `503d506`: it called a function defined nowhere and died of a `NameError` instead of reading the peer's log, which is why no failing run has ever said whether the peer saw a packet. **Exit criterion:** the gate passes on the runner, or the reason it cannot is named and the gate reports not-checked rather than failed. |
