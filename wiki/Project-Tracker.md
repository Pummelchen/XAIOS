...
**The booted-guest handshake gate is green in CI as well, on a Mac.** It reached its
host peer through QEMU's user-mode network, and `WT-PEER-TIMEOUT no Initial arrived`
read like a network fault -- while the same boot logged a DHCP lease and a validated
ARP reply from the gateway. It was not the network. `wt_peer --server` waited a fixed
twenty seconds for the client, the gate starts it *before* booting the image, and the
boot and handshake take 18 seconds here against considerably more on a shared runner,
so the peer had gone before the guest reached `/bin/wtqtest`. The wait is the caller's
budget now (B-138) and the gate gives it the same 300 seconds it gives the boot. The
gate has its own `macos-14` job -- the workflow's only macOS job -- and reports `the
guest completed a WebTransport handshake` there.
...
| `B-138` | The host peer waited twenty seconds for a guest that takes longer to boot | `TESTING` | `wt_peer --server` waited a fixed twenty seconds for the client's first datagram, and `qemu-quic-handshake-gate` starts the peer before it boots the image, so on a slow machine the peer had already timed out -- `WT-PEER-TIMEOUT no Initial arrived` -- when the guest reached `/bin/wtqtest`. Measured here, the boot and handshake together take 18 seconds, which is why it passed on this Mac and failed on both CI runners, and why the same failure appeared on Linux and on macOS. The wait is now the caller's budget (`XAIOS_WT_PEER_WAIT_SECONDS`, default unchanged for the interop test) and the gate passes its own 300 seconds; the peer prints the budget it was given, so a failure cannot be read as a network fault again. Verified locally end to end -- the gate reports a completed handshake with `wait_seconds=300` -- and the row closes when a CI run says the same. |
...
| `B-137` | The booted-guest WebTransport handshake gate failed on the Linux CI runner | `a7c080a` Run the guest handshake gate on a macOS CI runner (the failure's cause turned out to be `B-138`, the peer's fixed twenty-second wait, and not the runner's network) |
