...
| B-52 | A machine that has run long enough stops booting | all four | `OPEN` | `allocate_extents` in `kernel/fs/xaiboot_fs.c` is first-fit and gives up once a file needs more than `XBFS_V6_MAX_EXTENTS` (16, `:86`) extents, returning `XAIOS_ERR_NO_MEMORY` at `:1364`; `clone_extents` turns that into `XAIOS_ERR_IO` and `commit_snapshot` fails. It releases everything it claimed first, so nothing is corrupted -- the volume stays sound and simply cannot be snapshotted. A v5 volume with 8192 data sectors holding a repeatedly rewritten `/var/log/kern.log` reaches that state by ordinary use: this machine's is 64 boots deep, 1550 blocks of 8192 used, and `fsck` reports `errors=0 valid=1`. Free space is not the problem; the shape of it is. **The consequence is a machine that will not boot.** `kmain.c:1037` runs `update_self_test()` on every boot that has a writable persistent filesystem, and it asserts at `update.c:752` that `update_stage()` succeeds. Staging writes a snapshot, so on a volume this fragmented the assertion fires and the machine halts in the cyan screen. Not a failed update -- a failed boot, on a machine that was working yesterday. An earlier version of this row said `operations.c:237` and `update.c:752` disagree about whether the error is fatal. They do, but that is not a defect: `operations.c` is the production path and tolerating it is right, while `update.c:752` is a self-test and being strict is what a self-test is for. The defect is that the condition happens at all. **To close:** allocate largest-run-first rather than first-fit, which needs fewer extents for the same free space and changes no on-disk structure, and measure whether that is enough on a volume driven into fragmentation on purpose. If it is not, the choices are compaction or a wider extent table, and the second is a format change. Either way the gate comes first: a volume aged until `commit_snapshot` fails, which nothing here currently builds. |
