...
| C-19 | Standalone application ownership | `DONE` | `xapt`, `xaiosctl`, `nano`, `htop`, `pong`, and diagnostics are dedicated ELFs. Editor/game terminal modules are app-owned, `htop` consumes a paged generic control snapshot, and the kernel contains no private app command path. |
