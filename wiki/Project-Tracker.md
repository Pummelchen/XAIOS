...
| `B-136` | Source files run to thousands of lines, and the 500-line rule is not enforced | `IN PROGRESS` | 120 tracked files exceed 500 lines and 44 exceed 1000; the largest is `kernel/runtime/network_stack.c` at 5900. The limit is stated in `AGENTS.md` and on this page, and nothing measured it until `check-file-size-budget.py` joined `docs-check`: it fails when a new file crosses 500 lines and when an existing one grows past the size recorded for it, and it reports entries that have come back under the limit so the baseline can be cut. **The pre-existing `wt-host-test` defect is fixed (2026-09-18).**Fixing it exposed a second one behind it, in the same CI job: the interop peer compiled the vendored BearSSL tree with `-Werror`, and on Linux the POSIX entropy seeder's `getentropy()` is an implicit declaration under `-std=c99` without `_DEFAULT_SOURCE`, so the step failed in upstream's code. It was invisible because a job stops at its first failing step and the host tests above it failed first. Vendored sources now compile without `-Werror` and with the macro, which is the rule the host-test runner already followed.  It was recorded here as out of scope for the size campaign: the host runner's BearSSL subset carried only the *verify* half of the signature primitives, so the link failed on five symbols from the day the port started signing. Fixing it was a scope decision, not a split, and the decision was taken to get CI green before publishing build 7. Four sources were added and a second defect fell out from behind the red gate: the QUIC packet suite's split module used counter names the split had renamed, which nothing had compiled because the link failed first. All eight suites now pass, 1,572 checks. **Build 7 is cut.** With the size baseline empty, the release work started and two defects came out of it, both found by gates rather than by reading. The VMware Fusion chainloader step hung: `grub-mkstandalone` in a container with a bind mount of the whole tree never returned -- eighty minutes at 0.08s of CPU, no output, and no bound on the build. It is now fed through a pipe instead, same tool and same output. Then the release image gate found that **Fusion panicked** while QEMU booted: `klog`'s drop counter was read before the kernel was on its own page tables, and Fusion places the kernel high in RAM where QEMU does not. A bisect over the 333 commits since build 6, with an automated build-and-boot test, named the commit; resolving `ELR`/`FAR` against the kernel's symbols named the instruction and the variable. The counters are now armed after `vmm_init`, and the AArch64 boot mapper stops rather than writes past its L3 table array. `make release-image-gate` passes on four of the five environments -- the three QEMU ones and Fusion. **Apple Virtualization.framework is recorded as NOT CHECKED, not as passing:** the guest console never attached on this host, every marker failed together with no panic, and `make vmm-package-gate` fails only that kit. The notes say so in those words. One more thing worth recording: the file-size ratchet caught the fix itself -- the bounds guard pushed `mmu_boot.c` to 503 lines, and the comment had to come down to keep the file at 500. A twenty-second and final batch: the last ten files over the limit, all landed. **The size baseline is now EMPTY -- `check-file-size-budget.py` reports `0 files over 500 lines, none grown, and no new one`.** The campaign began with 120 baselined files. The last ten: `platform/qemu/run-qemu-aarch64.sh` 558 to 117 and `platform/qemu/run-qemu-riscv64.sh` 535 to 134 (both split into sourced helpers named so the two runners cannot collide), `tests/scripts/qemu-readiness-gate.py` 552 to 122, `kernel/fs/initramfs.c` 552 to 389 (with `INITFS_MAX_FILES` deliberately left where `qemu-abi-contract.py` reads it), `tests/scripts/qemu-ssh-connection-rate-gate.py` 544 to 253, `scripts/build-boot-media.sh` 544 to 207 (the third builder split, same byte-for-byte flattening proof), `tests/network/parallel-client-load.py` 541 to 401, `tests/scripts/qemu-power-loss-gate.py` 522 to 415, `kernel/dev/virtio/virtio_transport.c` 504 to 332 (the status/reset sequence, compiled for the MMIO backend only) and `tests/scripts/cluster_fault_relay.py` 502 to 235. **Completion evidence, checked independently of the checker:** every tracked file with a counted suffix (1211 of them) was scanned directly for line count -- none exceeds 500, and the largest is exactly 500. The repository checks agree, and the full verification pass passes on all three architectures. A twenty-first batch: TEN files, all landed, taking the baseline from 20 entries to **10** -- the last ten are all between 502 and 558 lines. `tests/security/test_wt_quic_pkt.c` 604 to 42, `tests/scripts/qemu-sshd-audit-append-gate.py` 602 to 291, `engine/src/packed.c` 601 to 389 (the SIMD kernels moved out, wired at nine sites across two architectures, the RISC-V kernel and three Makefile fragments), `tests/scripts/qemu-freebsd-network-suite.py` 598 to 267, `tests/repository/check-code-scanning-contract.py` 575 to 293 (its YAML reader split into a sibling the parent imports -- a repository check editing a repository check, with the verdict compared across 38 synthetic workflows), `tests/scripts/qemu-operations-closure.py` 570 to 224, `kernel/arch/x86_64/smp.c` 568 to 241 (the shootdown/idle self-tests, leaving SIPI ordering untouched -- this is the file whose AP path has bitten us), `userspace/lib/xaios_screen.c` 564 to 461, `tests/scripts/qemu-cpu-matrix.py` 559 to 185 and `tests/scripts/qemu-installed-disk-gate.py` 558 to 379. One wiring mistake was mine and the linker caught it: a Makefile pattern that was a substring of another line got `packed_simd.c` added twice, and the duplicate symbol was reported by the hosted test link. A twentieth batch: eight files, all landed, and **eight more left the baseline**, taking it from 28 entries to **20** -- a sixth of where this campaign started. The first ASSEMBLY file was split: `kernel/arch/riscv64/entry.S` 610 to **374**, moving the independent global entries after the trap vector into `entry_helpers.S` (294) compiled as a second object. Assembly has no headers and no `static`, so the seam is the linker: the trap vector's branch to `riscv64_return_from_user_exit` is an `R_RISCV_BRANCH` (+/-4 KiB) with `-mno-relax`, which means the two objects must stay ADJACENT in the link -- the compile line was inserted immediately after entry.S for exactly that reason, and the RISC-V boot plus the AIA and IOMMU gates pass. Also off the baseline: `userspace/apps/xaios-setup.c` 633 to 364, `tests/network/debian13-client-suite.sh` 633 to 78 (two sourced helpers, with the Dockerfile COPY lines added because the container sources them), `kernel/storage/gpt.c` 625 to 402, `tests/scripts/vmware-fusion-load-soak.py` 613 to 393, `kernel/net/dhcpv6.c` 609 to 158, `tests/scripts/qemu-cluster-three-node-gate.py` 608 to 410, and `kernel/arch/riscv64/fdt.c` 613 to 154. **The fdt split arrived half-finished:** the agent for it failed and reported nothing, but its edits had landed, leaving `fdt_lookup.c` at 510 lines -- still over the limit -- and the RISC-V link broken on three undefined symbols. I finished it by hand, moving the interrupt-controller group into `fdt_intc.c` and exporting the two helpers it shares, then wired both files. **A failed agent is not an agent that did nothing**, and the link is what said so. A nineteenth batch: eight files, all landed, and **eight more left the baseline**, taking it from 36 entries to **28** -- the campaign is now four-fifths of the way from the original 120. `kernel/fs/xai_fs_admin.c` 676 to 377, `kernel/net/ipv6.c` 675 to 335 (extension headers and reassembly), `kernel/runtime/source_index.c` 664 to 345 (scanner and self-test), `scripts/build-vm-packages.sh` 661 to **124** (two sourced helpers, with the same flattening proof as the other two builders, reproduced independently after the split), `kernel/arch/riscv64/smp.c` 661 to 307 (bring-up and registry, mirroring aarch64), `tools/xaios_model_v2.py` 640 to 235, `tests/scripts/qemu-smoke.py` 636 to 237 (the marker tables moved out, so the gate a dozen other gates call is now a thin driver) and `kernel/core/klog.c` 635 to 339 (console transport and formatter). **`make vm-package-gate` could not be run and is recorded as NOT CHECKED, not as passing:** it needs `release/xaios_b6-aarch64.iso`, which this checkout does not carry (`make vm-packages` stops with `run: make release-package`). The split was instead verified by syntax-checking the parent and both helpers under bash and sh, and by independently reproducing the byte-for-byte flattening proof. A gate that cannot run is reported not checked, never quietly counted as green. An eighteenth batch: eight files, all landed, and **eight more left the baseline**, taking it from 44 entries to **36**. The one that mattered most: `kernel/arch/x86_64/early.c` 692 to **469**, by moving **the AP startup itself** -- `x86_64_ap_entry`, `patch_ap_trampoline` and `start_application_processors` -- into `early_ap.c`. An earlier attempt at exactly this extraction panicked the x86_64 boot during AP startup and was reverted, so it was retried last among the boot stages and only after the seam was reduced to four scalars that cross as accessors. The x86_64 smoke gate passes, which is the marker set that failed before. Also off the baseline: `kernel/dev/pci_ecam.c` 691 to 475 (BAR assignment and MSI-X), `tests/security/test_wt_crypto.c` 688 to 48, `tests/scripts/qemu_gate_lib.py` 688 to 347 (architecture and terminal helpers, with every name re-exported so no importer changed), `kernel/core/panic.c` 685 to 289, `tests/scripts/qemu-parallel-network-load.py` 684 to 296, `tests/scripts/qemu-outbound-batch-mode-gate.py` 680 to 286 and `kernel/include/xaios/control_protocol.h` 678 to 113 (split into codes and payloads sub-headers, byte-for-byte, with no name or wire value changed). **A second check was edited for the same reason as last round's: `check-panic-registers.py` read the register renderer out of `panic.c`, and the renderer now lives in `panic_render.c`.** The assertion is unchanged; only the file it reads moved. Two checks have now needed that treatment, which makes it a pattern rather than an incident: these checks pin an implementation LOCATION as well as a property, so any split that moves the code they read must update them. A seventeenth batch: eight files, all landed, and **seven more left the baseline**, taking it from 51 entries to **44**. The RISC-V image builder got the same treatment as the AArch64 one: `scripts/build-riscv64-image.sh` 741 to **63**, by extracting eleven blocks into `scripts/lib/riscv64-*.sh` (24 to 173 lines each) sourced in place, with the same mechanical proof -- expanding the source lines reproduces the original script byte for byte. That file had been growing with every wiring pass in this campaign, so it was the wiring itself that made it a target. Also off the baseline: `kernel/dev/input.c` 745 to 360, `kernel/arch/x86_64/mmu.c` 732 to 322, `tests/scripts/qemu-sshd-transmit-rate-gate.py` 724 to 358, `kernel/runtime/git_workspace.c` 697 to 437, `tests/network/qemu-ipv6-tcp-client.py` 695 to 201 and `tests/scripts/qemu-cluster-partition-gate.py` 692 to 341. `kernel/arch/x86_64/early.c` 780 to 692. **One gate was edited this round, deliberately: `qemu-x86_64-numa-gate.py` read the IDT trap-gate idiom out of `early.c`, and the IDT install now lives in `early_idt.c`.** The assertion is unchanged -- the syscall vector must still be an interruptible trap gate -- and only the file it reads moved, with a comment saying why. This is the recorded trap firing: the check pins an implementation location, not just a property, so any split that moves `install_idt` breaks it. Also worth recording: the object name `mmu_boot.o` is used by BOTH architectures from different source directories, so adding the x86_64 pair to the object lists put a duplicate into the aarch64 list and the linker reported duplicate `a64mmu_*` symbols. A sixteenth batch: eight files, all landed, and **seven more left the baseline**, taking it from 58 entries to **51**. **`sshd.c` is off the baseline**: 583 to **130**, by moving `process_connection` whole into `sshd_connection.c` -- the same technique that worked for `sshd_run`, and the reason last round's pessimism was wrong. I had said reaching 500 required splitting the function's phases into a state struct; it did not, because moving a function whole to its own translation unit is not the same as splitting it. No accessor was needed and the body is byte-identical (`git numstat` 0 added, 453 removed). The lesson generalises: **the campaign's three 'do not split' hubs were a rule about restructuring bodies, and I had been reading it as a rule about file layout.** Also off the baseline: `kernel/arch/aarch64/smp.c` 797 to 464 (platform and registry, with the shared globals prefixed `a64smp_`), `kernel/runtime/update.c` 774 to 393, `kernel/runtime/ai_cell.c` 770 to 443, `kernel/fs/vfs.c` 757 to 181 (namespace and handle modules, lock discipline unchanged), `kernel/runtime/security.c` 753 to 436 (update-signature and credential paths) and `tests/security/test_wt_tls.c` 749 to 40. `kernel/arch/x86_64/early.c` 812 to 780. Two wiring notes: `smp.o` appears in both architecture object lists, so inserting after it needs the aarch64 context rather than the string; and `scripts/build-riscv64-image.sh` is now 741 lines -- the same kind of file `build-image.sh` was before it was split into sourced helpers, and it now needs that treatment. A fifteenth batch: eight files, all landed, and **seven more left the baseline**, taking it from 65 entries to **58**. **The sshd hub rule was relaxed, deliberately.** `sshd.c` 1034 to 583 by moving `console_tick` and `sshd_run` whole into `sshd_service.c` -- a move, not a refactor: bodies unchanged, state reached through the accessors the other sshd modules already publish, and `g_server_stats`/`g_connection_close_count` moved with them so no mutable state crossed. The move is verified by the boots and by `qemu-ssh-smoke`, which drives the real SSH path. **`sshd.c` is still over 500 and cannot reach it while `process_connection` (448 lines) stays whole** -- 583 minus the non-hub remainder is exactly that function, so the next step is splitting its phases, and that is a refactor with a state struct rather than a move. Also off the baseline: `kernel/dev/virtio/virtio_transport_pci.c` 835 to 225 (probe and queue), `kernel/arch/riscv64/aia.c` 833 to 337 (init and self-test), `kernel/core/kmain.c` 832 to 451 (platform and runtime bring-up), `tests/scripts/qemu-freebsd-bidirectional-suite.py` 831 to 436, `kernel/sched/thread.c` 824 to 337 (user-thread ops and self-test), `kernel/arch/riscv64/iommu.c` 819 to 390 (hardware programming and device directory) and `tests/scripts/qemu-model-sftp-gate.py` 814 to 488. The x86_64 virtio transport is compiled into `virtio_transport.o` rather than a PCI object, so the two new objects had to be added to the **x86_64** object list as well as the aarch64 one -- the link caught it. A fourteenth batch: eight files, all landed, and **seven more left the baseline**, taking it from 72 entries to **65** -- `tests/xai_fs/test_xai_fs_reader.c` 888 to 220, `kernel/runtime/operations.c` 857 to 219, `engine/src/xai_fs.c` 856 to 403, `tests/scripts/qemu-console-xtop-gate.py` 840 to 313, `userspace/lib/xaios_user.c` 839 to 371, `tests/control/test_control_client.c` 838 to 312 and `userspace/include/xaios_control.h` 835 to 126. The header needed its own rule: it was split into three included sub-headers with **no name, order or spelling changed**, because a header split that renames anything is an ABI change wearing a tidy-up's clothes. `xaios_user.c` is the widest wiring of the campaign -- eleven build sites across the image builder, the RISC-V image, the standalone app builder, the WebTransport app builder and three shared fragments -- and the link caught two of my own errors (a `$CFLAGS` that does not exist in one builder and a duplicated compile block). Also `kernel/arch/x86_64/early.c` 945 to 812 (the LAPIC/CPU access layer, its eighth module, wiring unchanged). A thirteenth batch: eight files, all landed, and **seven more left the baseline**, taking it from 79 entries to **72**. The RISC-V MMU cut that was reverted last batch was redone successfully: `kernel/arch/riscv64/mmu.c` 1018 to 399 with the file-scope state moved whole into `mmu_map.c` (which is now the only writer and owns `vmm_init`) and the parent reaching the root **by address, never a pointer or a shadow copy** -- which is exactly the property whose absence made the first attempt fault on a null root. `vmm_translate`, `vmm_validate_user_buffer`, `riscv64_hart_satp` stay defined in the parent, and the RISC-V boot gate passes. Also off the baseline: `userspace/wt/src/wt_tls_handshake.c` 993 to 310, `kernel/mm/numa.c` 990 to 457, `tests/security/test_wt_tls_cert.c` 979 to 128, `kernel/net/ndp.c` 966 to 246, `kernel/runtime/app_store.c` 932 to 406 and `userspace/apps/xapt.c` 931 to 418. `kernel/arch/x86_64/early.c` 1010 to 945 (the end-of-bring-up contract reports, its seventh module). **A defect of mine was found while wiring this batch:** in an earlier round I added the per-test extra-source case to `tests/security/run-wt-host-tests.sh` and then reverted the file with a `git checkout` while undoing an unrelated probing edit, which silently dropped the wiring for two split WT suites. It went unnoticed because that suite's link failure is pre-existing; the case block is restored and now covers three suites, and the split files resolve. A twelfth batch: eight files attempted, seven landed, and **five more left the baseline**, taking it from 84 entries to **79** -- `userspace/sshd/ssh_channel.c` 1031 to 469 (channel table and request dispatch), `userspace/sshd/sftp_server.c` 1009 to 279, `tests/scripts/vmware-fusion-outbound-gate.py` 1005 to 411, `kernel/arch/aarch64/mmu.c` 1003 to 385 and `kernel/runtime/ai_kernels.c` 997 to 287; also `kernel/arch/x86_64/early.c` 1124 to 1010 (the TSC/PIT calibration, its sixth conservative module) and `userspace/sshd/sshd.c` 1093 to 1034. **One extraction was reverted, and the way it failed is worth remembering.** `kernel/arch/riscv64/mmu.c` came back at 419 lines with the link clean, but the extraction had silently *deleted* three public entry points rather than moving them (`vmm_translate`, `vmm_validate_user_buffer`, `riscv64_hart_satp`); restoring them from the parent commit turned the link green and the boot red -- a load fault on a null root during VMM initialisation, which `resolve-panic.py` plus `llvm-addr2line` placed inside mmu.c. The other seven files were untouched by it. Reverted to the parent revision, wiring removed, and the RISC-V boot gate passes again; the split needs a redo whose entry points are preserved by construction rather than recovered after the link complains. Also recorded here: `sshd.c` cannot reach 500 lines under the constraint that its three hubs stay whole -- those hubs are 885 lines, so the floor is about 915, and the constraint has to change before the file can. An eleventh batch: eight files, and **seven more left the baseline**, taking it from 91 entries to **84**. The last build file went the same way as the first: `Makefile` 1656 to **91**, by extracting eight target groups into `mk/*.mk` fragments (43 to 429 lines) included from the parent. The proof here is stronger than a diff: all 244 target definitions and all 315 `make -pn` entries are identical to before, and every target dry-runs byte-identically. The parent keeps the variable preamble and the fourteen targets whose names are matched as literal text by repository checks, because moving those would have changed what the checks see. Also off the baseline: `userspace/lib/xaios_control_client.c` 1171 to 421 (argument parsing split by flag and by validator, plus the dispatch), `scripts/xaios-ssh-bridge.py` 1171 to 489, `userspace/wt/src/wt_tls_client.c` 1078 to 447, `kernel/dev/vmxnet3.c` 1066 to 303, `kernel/runtime/cpu_ai_runtime.c` 1055 to 423 and `kernel/arch/riscv64/exception.c` 1043 to 489. `kernel/arch/x86_64/early.c` 1168 to 1124. Two defects were caught and fixed during wiring: the extracted dispatch passed `&offset` where the paramter is already a pointer (twenty-one call sites), and a replacement of mine again appended link objects to a compiler's `-o` line. A tenth batch: eight files, and **seven more left the baseline**, taking it from 98 entries to **91**. The largest file in the repository is now split: `scripts/build-image.sh` 2121 to **398**, by extracting ten verbatim blocks into `scripts/lib/*.sh` (78 to 285 lines each) sourced from the parent in the same order. A flattening check -- replacing each source line with the body of the file it sources -- reproduces the original byte for byte, which is what makes a shell refactor like this safe to claim. The kernel's object list and compile list moved with it, so continuing splits now wire `scripts/lib/kernel-objects.sh` and `kernel-compile.sh` rather than the parent. Also off the baseline: `network_stack.c` 1849 to 372 (the TCP flow table stays put and is passed to six new modules as a caller-owned pointer, so no 17 KB row ever crosses -- the constraint recorded last batch turned out not to block the cut after all), `userspace/apps/xtop.c` 1203 to 288, `kernel/sched/scheduler.c` 1202 to 492, `kernel/runtime/control_protocol.c` 1197 to 464, `kernel/fs/vfs_xaifs.c` 1196 to 423 and `kernel/storage/storage_admin.c` 1184 to 465. `kernel/arch/x86_64/early.c` 1366 to 1168 (the platform/idle layer, its fifth conservative module). The build-script refactor raced the agents reading the old line numbers, so all kernel wiring landed in the new `scripts/lib/` files instead; two of my own wiring omissions were caught by the linker. A ninth batch: eight files, nineteen modules, and **six more files left the baseline** -- `kernel/runtime/admin_control.c` 1310 to 493 (configuration and the authorized-key record), `userspace/apps/xutils.c` 1284 to 490 (file utilities and text utilities), `kernel/dev/virtio/virtio_blk.c` 1282 to 443 (request engine and backend/self-test), `kernel/net/dns.c` 1241 to 349 (resolver state machine and the deterministic self-test), `kernel/fs/fat.c` 1238 to 422 (directory traversal and the file I/O path) and `userspace/sshd/ssh_crypto.c` 1205 to 365 (symmetric ciphers and Curve25519/Ed25519, with SHA-256/512 deliberately left unguarded because the hashes-only UEFI loader compiles that file alone). The baseline is **98 entries**, from 120 when this campaign began. Also `network_stack.c` 2098 to 1849 -- the TCP statistics self-check and the boot lifecycle -- and `kernel/arch/x86_64/early.c` 1544 to 1366, its fourth conservative module (the interrupt-entry C dispatch, chosen for its position with the IDT and AP path untouched). One structural limit stated plainly: the TCP frame handlers and the flow table stay put because a flow row is about 17 KB, so copying one per frame would overflow the 16 KB secondary stacks -- that is a hardware constraint, not a tidiness preference. An eighth batch: eight files, sixteen modules, and **five more files left the baseline** -- `kernel/runtime/remote_login.c` 1356 to 458 (the session/account layer and the boot-test archive container format), `kernel/dev/virtio/virtio_net.c` 1347 to 461 (transmit path and queue/feature setup), `kernel/user/service.c` 1340 to 448 (registry and lifecycle, with the locked public entry points left in place so lock scope is unchanged), `kernel/core/boot_ui.c` 1337 to 436 (framebuffer render and terminal emulation, with the geometry constants a console gate parses left in place) and `userspace/apps/clustertest.c` 1317 to 394 (frame transport and the mesh loop). The baseline is 104 entries, from 120 when this campaign began. Also `network_stack.c` 2400 to 2098 -- the poll plane moved whole, because splitting the loop from its tail would have shifted the early-return skips -- and `early.c` 1583 to 1544, its third conservative module (GDT/TSS), chosen again for its boot position with the AP path untouched. `sshd.c` is 1327 to 1093 and cannot reach 500 while its three hubs are whole: they are 884 lines by themselves. The clustertest split is verified by `make qemu-cluster-partition-gate`, which runs the application. A seventh batch: eight files, nineteen modules, and **six more files left the baseline** -- `kernel/fs/xaiboot_fs.c` 1411 to 467, `kernel/dev/nvme.c` 1222 to 419, `userspace/sshd/ssh_client.c` 1230 to 417, `tests/security/test_wt_tls_handshake.c` 1646 to 498, `tests/security/test_wt_tls_client.c` 1402 to 484 and `tests/scripts/qemu-docker-network-suite.py` 1451 to 496, taking the baseline from 115 entries to 109. Also `network_stack.c` 2902 to 2400 (UDP receive and the ARP/ICMP/ping handlers, with the caller passing the interface MAC in) and `kernel/arch/x86_64/early.c` 1748 to 1583 (xsave/XCR0 validation and the nested IRQ-state areas, chosen for its boot position and with the x86 boot gate passing). **A pre-existing defect found while verifying, not caused by this work:** `make wt-host-test` fails to link every test because the module list in `tests/security/run-wt-host-tests.sh` includes `userspace/wt/xaios/wt_xaios_x509.c`, which needs BearSSL's RSA and X.509 decoder chain that the runner deliberately does not compile. It predates this campaign -- the file is in the runner at the commit before it started, and the untouched `crypto` test fails identically. Fixing it means either extending the runner's BearSSL subset (whose decoder sources are generated from templates) or taking the x509 module out of the host test link, which is a decision about what that gate covers, so it is recorded rather than guessed at. A sixth batch: eight files, sixteen modules, and **three files left the baseline at once** -- `boot/uefi/loader_main.c` 1579 to 472, `kernel/user/user.c` 1512 to 484 and `engine/src/xai_fs_writer.c` 1455 to 496, so 118 entries became 115. Also: `network_stack.c` 3583 to 2902 (the app/external-session plane and the boot self-test, both leaves that own no state); `kernel/fs/xaiboot_fs.c` 1765 to 1411 (block I/O and mount state, and commit/rollback); `kernel/arch/riscv64/mmu.c` 1677 to 1018 (remote TLB shootdown and the large-page self-test, with three named probes instead of the walk state); `tests/scripts/qemu-cluster-partition-gate.py` 1511 to 692 (protocol and analysis helpers; it stays over 500 because the mesh grammar, port table and relay harness are one runtime). `early.c` was retried **conservatively** and landed this time: exactly one module, the ACPI/MADT parse and CPU-record preparation, chosen because it runs once, single-threaded, before paging, with no coupling to AP startup -- the path that panicked last round -- and the x86_64 boot gate passed. Two wiring defects the pass caught: a new loader header whose opening comment was never closed (which swallowed its includes) and engine objects added to the link under the wrong names. A fifth batch: seven files landed, fourteen modules, and one file was reverted. `network_stack.c` 4229 to 3583 (the packet-descriptor pool, where a packet is a 1-based lease and never a pointer, and the UDP table and data plane); `xaiboot_fs.c` 2350 to 1765 (the byte-level file data path and the descriptor layer, taking `g_open_files` and the staging buffer with their only users); `control_protocol.c` 1964 to 1197 (the six fixed-size read-only operations and the storage-layout group); `sshd.c` 1959 to 1327 (diagnostics and the console UI, with the three hubs still whole); `remote_login.c` 1722 to 1356 (the parse/output primitives and the stat/mkdir/touch/write handlers); `kmain.c` 1693 to 832 (boot storage and app launch, with every boot message, order and capability mask unchanged); `ssh_channel.c` 1632 to 1031 (wire framing and the shell/exec path). **`early.c` was reverted**: its two extracted modules passed compile and link but the x86_64 boot panicked with an empty message in the AP-startup path -- `qemu-x86_64-smoke` caught it, which is exactly why that gate is in the pass. The file is 1793 lines and is back to its known-good state; it needs a round of its own with the x86 boot gate run before anything else is built on it. A fourth batch, eight files, twenty-one modules, and a second file off the baseline: `network_stack.c` 4982 to 4229 (TCP data plane, chosen over the receive dispatch because the dispatch lives inside the poll loop and reaches a dozen objects); `xaiboot_fs.c` 2982 to 2350 (namespace and allocation groups, with `g_xbfs` behind value accessors and two documented one-call pointers); `sshd.c` 2687 to 1959 (authentication and key management; the console login state machine stayed because `console_tick` shares its state); `early.c` 2479 to 1793 (memory setup and PCI scan); `remote_login.c` 2335 to 1722 (copy commands and the application launcher); `xtop.c` 2189 to 1203 (glyph canvas and the 415-line dashboard); `ssh_client.c` 2128 to 1233 (KEX core and handshake); and `kernel/user/syscall.c` 2157 to 418, whose 1825-line dispatch chain became seven family modules -- that file leaves the baseline entirely, 119 entries to 118. One defect the pass caught and fixed: moving the x86 ring-3 probe to a new file made it non-static, so the compiler emitted it and its `_binary_hello_bin_*` references in the common-runtime build, where that embedded image is not linked; the original guard was restored around it. Everything else linked first try; the three missing shared defines the compiler found (two process-state aliases and a relay socket constant) were fixed the same way. A third parallel batch, eight more files, and the first file to leave the baseline entirely: `kernel/runtime/network_stack.c` 5187 to 4982 (its IPv6 address path moved to `network_stack_v6.c` behind fourteen copy-in/copy-out accessors -- the receive dispatch was rejected because it reaches both flow tables, the packet descriptors, the queue rings and ping state); `kernel/fs/xaiboot_fs.c` 3175 to 2982 (metadata buffer, shadow, journal, with the buffer accessor documented as valid for one call and the scalars behind get/set); `userspace/sshd/sshd.c` 2800 to 2687 (console program launch; `console_execute_command` stayed because it would have meant handing out pointers to mutable state); `kernel/arch/x86_64/early.c` 2570 to 2479 (the serial block, which needed no accessors at all); `userspace/apps/xtop.c` 2525 to 2189 (panels and gauges; the 415-line dashboard stayed because it cannot fit); `kernel/runtime/remote_login.c` 2508 to 2335 (pager and pipeline driver, with `parse_and_execute` kept because it owns the catalog line a check reads); `kernel/arch/riscv64/mmu.c` 2033 to 1677 (remote-shootdown self-test). And `tools/xaios_xai_fs.py` 2042 to 297: it is now a facade over six modules each under 500 lines, with `_read_candidate`, `_decode_superblock` and the CLI still working, so it is the first file to leave the baseline -- 120 entries became 119. Every architecture linked first try this time and the pass covered all three links, three smoke boots, the filesystem and SSH gates and docs-check. Thirty-second to fortieth modules landed, again as one parallel batch verified once: `kernel/arch/x86_64/early_tlb.c` (368) and `early_cpu.c` (125) out of `early.c` (2964 to 2570); `userspace/sshd/sshd_console_screen.c` (149) out of `sshd.c` (2933 to 2800); `kernel/runtime/control_ops.c` (400) out of `control_protocol.c` (2667 to 1964 across three cuts); `kernel/dev/nvme_queue.c` (290) out of `nvme.c` (1701 to 1222); `kernel/fs/vfs_xaifs_scrub.c` (270) out of `vfs_xaifs.c` (1798 to 1196); `kernel/dev/virtio/virtio_net_selftest.c` (229) out of `virtio_net.c` (1683 to 1347); and `userspace/lib/control_render_config.c` (257) plus `control_request.c` (310) out of `xaios_control_client.c` (3589 to 1171 across six cuts). Two hard lessons, both recorded here because they cost real time: **the verification pass must link all three architectures** -- a previous round's dedup of build-list lines silently deleted the x86_64 entries and nothing noticed until this round; and two agents must never be given the same parent file concurrently, which one batch did with `early.c` (both edits survived, but only by luck, and one function needed repair). The batch method still wins -- nine modules for one gate pass -- but the pass must be: all three links, hosted tests, a boot gate per architecture touched, and the area gates. Twenty-third to thirty-first modules landed in one batch, and the method changed to do it: instead of one extraction per build-and-gate cycle, nine extractions were done in parallel across disjoint files and the whole set was verified once. `kernel/user/syscall_table.c` (196) takes the syscall table out of `syscall.c` (2345 to 2157); `userspace/sshd/ssh_client_scp.c` (339) the SCP-over-SFTP path out of `ssh_client.c` (2803 to 2128 across both cuts); `userspace/apps/xutils_archive.c` (482) the tar/zip commands out of `xutils.c` (1718 to 1284); `kernel/runtime/remote_login_tar.c` (303) and `remote_login_zip.c` (480) the tar and zip/cpio handlers out of `remote_login.c` (4883 to 2508 across five cuts); `userspace/apps/xtop_serve.c` (495) the serve session out of `xtop.c` (3065 to 2525); `kernel/runtime/control_storage_ops.c` the storage operations out of `control_protocol.c` (2667 to 2311); `kernel/dev/virtio/virtio_blk_handles.c` the handle path out of `virtio_blk.c` (1800 to 1282); and `kernel/fs/vfs_xaifs_trim.c` the trim half out of `vfs_xaifs.c` (1798 to 1440). Roughly 3500 lines left the eight largest remaining files in one round. Two failures were caught by the single verification pass and fixed: the wiring had been applied twice for the kernel modules (duplicate objects in the link list), and the xtop module needed `userspace/sshd` on its include path. Twentieth, twenty-first and twenty-second modules landed, the first three from the next tier: `engine/src/xai_fs_writer_util.c` (239 lines) takes the codecs, checked arithmetic, digest and validators out of `engine/src/xai_fs_writer.c` (1672 to 1455); `userspace/sshd/ssh_sftp.c` (287) takes the SFTP client block out of `userspace/sshd/ssh_client.c` (2803 to 2460); `userspace/sshd/ssh_alt_screen.c` (267) takes the alternate-screen programs out of `userspace/sshd/ssh_channel.c` (1883 to 1632). Three text-reading traps were respected: the chunk-size defines stay in `xai_fs_writer.c` with their only user `valid_chunk_size`, the install literals stay in their file, and the shell catalog stays in `remote_login.c`. The baseline again passed with no update. **A tooling gap found while planning this tier, recorded rather than left to be discovered:** `make compile-check`'s architecture sweep compiles `kernel/arch/x86_64/*` with `-fsyntax-only` and without `-DXAIOS_X86_COMMON_RUNTIME=1`, so a new file's `XAIOS_X86_COMMON_RUNTIME` arm is compiled but never checked -- the same class of blind spot as `compile-check` not linking, which has already cost two link failures.  Eighteenth and nineteenth modules landed, both of them unblocked by that accessor work. `kernel/fs/xbfs_record.c` (181 lines) takes the record builders and the six on-disk record descriptions out of `kernel/fs/xaiboot_fs.c`, which drops from 3890 to 3175 across its four steps; `write_file` crossed as `xbfs_write_file_locked` and each descriptor is declared `extern const char k_x[sizeof(...)]` so the self-test's `sizeof` stays byte-identical. `kernel/runtime/remote_login_sysinfo.c` (334 lines) takes `ps`, `df`/`du` and `xtop`'s state helpers out of `kernel/runtime/remote_login.c`, which drops from 4883 to 3268 across its four extractions. The baseline passed with no update again -- both files shrank. Verified by `make compile-check`, `make hosted-test`, `make image-qemu-test`, `make qemu-filesystem-gate`, `make qemu-smoke`, both guard configurations, `make riscv64` and `make docs-check`.  `kernel/fs/xbfs_state.c` (142 lines) now owns xaibootFS's counters and geometry behind accessors: 23 counters indexed by an `xbfs_stat_t` enum with get/bump/add/reset_all, and the seven `g_active_*` objects behind `xbfs_geometry_t` with a getter per field, a snapshot read for functions that want several, and `xbfs_geometry_select()` replacing the five `set_active_v2..v6` bodies. 96 increment sites, 23 read sites and 16 geometry writers were rewired; the self-test now asserts through the interface instead of poking state. `xaiboot_fs.c` drops from 3890 to 3339 across its three steps, and the baseline passed without an update because the file shrank. One tension worth knowing: the text check requires the `XBFS_V*_METADATA_SECTORS` defines to stay in `xaiboot_fs.c`, so `xbfs_internal.h` repeats them for the module that now needs the same numbers -- identical tokens, and any drift is a macro-redefinition error rather than a silent disagreement. Verified by `make compile-check`, `make hosted-test` (six xaiboot host links), `make image-qemu-test`, `make qemu-filesystem-gate` and `make docs-check`. Sixteenth and seventeenth modules landed: `userspace/lib/control_render_storage.c` (460 lines) takes the storage, filesystem and partition renderers and `userspace/lib/control_render_ops.c` (441) takes install, partition plan, mutation, model cleanup, volume report, scrub and trim, out of `userspace/lib/xaios_control_client.c`, which drops from 3589 to 1755 across its four extractions. Config and authorised-keys stay because they are not storage; audit stays because it is the remainder that keeps both new files under the limit. The install verb's literals are still in the client and its assertion was run by hand (`make docs-check` does not include that gate, which the wiring note had implied). Verified by `make compile-check`, `make hosted-test`, `make image-qemu-test`, `make riscv64` -- the RISC-V link that a previous round found missing objects -- and `make qemu-smoke`. Fifteenth module landed: `kernel/fs/xbfs_node_codec.c` (166 lines) takes the v3/v4/v5 node import and export out of `kernel/fs/xaiboot_fs.c`, which drops from 3890 to 3434 across its two extractions. The analysis that picked this cut over building the whole accessor layer first was right: the codec touches zero file-scope state, so it moved with its types into `xbfs_internal.h` and changed no call site. Both text-reading checks were treated as hard constraints and verified rather than assumed -- the `XBFS_V*_METADATA_SECTORS` defines and the two literals `check-code-scanning-contract.py` requires are byte-identical in `xaiboot_fs.c`, and `extents_from_blocks`/`extents_to_blocks` were de-static'd but not moved, because two gates read them out of that path. Verified by `make compile-check`, `make hosted-test` (six xaiboot host links), `make image-qemu-test`, `make qemu-filesystem-gate` and `make docs-check`. Fourteenth module landed, and it is the one the file said could not be cut until its state was behind accessors. `kernel/runtime/network_stack_listener.c` (450 lines) takes the listener registry, the accept queue and the socket map out of `kernel/runtime/network_stack.c`, which drops from 5900 to 5157 across its two extractions. The accessors are a cursor plus a commit (`network_listener_slot_read/_write`, the same shape for the socket map) rather than a `find` handing back a row pointer, because the file had already rejected that form where a concurrent close could clear a row between the lookup and the dereference. Every accessor is lock-free and documents that the caller holds the guard; the two UDP delivery blocks that held a listener row across a re-entrant enqueue now copy it out first. The work confirmed no row pointer actually survived a depth-0 unlock, so no critical section was narrowed -- narrowing one would have turned an atomic enqueue into a lost update. Verified by `make compile-check`, `make image-qemu-test`, `make qemu-network-full-gate` (listeners, accept, socket mapping, UDP and TCP paths) and `make qemu-smoke`. Eleventh, twelfth and thirteenth modules landed. `userspace/lib/control_render_system.c` (480 lines) takes the version, status, health, capabilities, hardware, metrics and log renderers out of `userspace/lib/xaios_control_client.c`, which drops from 3589 to 2630 across its two extractions; the cut is at `render_logs` because version-to-configuration is 522 lines, past the limit. `userspace/sshd/sshd_rate_limit.c` (132) takes the connection-rate table and `userspace/sshd/sshd_kex.c` (443) the key-exchange and connection-crypto code, from `userspace/sshd/sshd.c`, which drops from 3756 to 2933 across its two extractions; `process_connection`, `sshd_run` and `console_tick` are still whole, as the earlier note requires. **A defect from an earlier split was found and fixed here:** the RISC-V links for `xaios-shell` and `xapt` were never given the control-primitives object, so they could not resolve `bytes_copy` -- the image had not been built since that split, and the new links now carry both control objects. Grown build files (the `Makefile` and the two image builders) were re-recorded in the baseline; a split's wiring is the one thing allowed to grow them, deliberately. Ninth and tenth modules landed: `kernel/runtime/remote_login_text.c` (492 lines) takes `grep`, `head`/`tail` and `sed`, and `kernel/runtime/remote_login_path.c` (491) takes `ls`, the two `find` forms and `remote_path_resolve`; `kernel/runtime/remote_login.c` drops from 4883 to 3576 across the three extractions. Five helpers that only a moved command used stayed behind (three for `head`/`tail`, one for `find`, and `glob_match` behind it): moving them pushed the modules to 538 and 540 lines, past the limit, so the smaller cut won and they are declared in the private header instead. `remote_path_resolve` is unguarded and the shipped shell calls it, so it and its primitives sit outside `XAIOS_BOOT_TEST_APPS` while the listing commands stay inside it. Verified by `make compile-check`, both guard configurations linking, `make qemu-smoke`, and the duplicate-symbol sweep over every kernel object. Eighth module landed: `kernel/runtime/remote_login_archive.c` (393 lines) takes the archive builder, extractor, lister and the ustar walker out of `kernel/runtime/remote_login.c`, which drops from 4883 to 4515. They are not self-contained -- they call 19 primitives that stay behind -- so those became module-internal exports rather than moving too. The current directory stays private behind a `remote_login_cwd()` accessor, and the archive magic is used by `handle_cpio` on the other side, so it is one definition with an `extern` declaration. The whole unit sits under `XAIOS_BOOT_TEST_APPS`, so both configurations were built: `image-qemu-test` (guard on, compiles and links the new code, booted by `qemu-smoke`) and the shipped guard-off image. No gate drives these commands directly -- the only tar/cpio marker belongs to the userspace shell -- which is worth knowing before trusting a green boot here. Seventh module landed: `userspace/sshd/sshd_audit.c` (305 lines) takes sshd's logging and audit path -- the durable-write counters, the buffered audit records and `ssh_log` -- out of `userspace/sshd/sshd.c`, which drops from 3756 to 3478. Four call sites reached into the moved state, so the module exports `ssh_audit_write_ns`, `ssh_audit_pass_durable_ns`, `ssh_audit_pass_reset`, `ssh_audit_flush` and `ssh_audit_buffered`; the per-close cost line spans both sections, so the key-cache counters cross it as arguments rather than as exported state. `-Os` follows the code into the new unit. Verified by `make compile-check`, `make image-qemu-test` (the link) and `make qemu-ssh-smoke`. Sixth module landed: `kernel/fs/fat_codec.c` (159 lines) takes fatFS's on-disk codec -- the entry layout, the 8.3 and long-name encoders and the byte-order helpers -- out of `kernel/fs/fat.c`, which drops from 1394 to 1238. **A trap found the hard way:** both this module and `xbfs_util.c` exported `bytes_zero`/`bytes_copy`, which `make compile-check` cannot see because it never links; `make image-qemu-test` failed with a duplicate symbol. Both modules' exports are now namespaced (`fat_*`, `xbfs_*`), and a kernel split is only verified once the image has linked. Fifth module landed: `kernel/user/syscall_socket.c` (276 lines) takes the kernel socket table out of `kernel/user/syscall.c`, which drops from 2610 to 2345. The table owns its row array, its capacity, the id and ephemeral-port counters, the connection count and the lock over them, so it moved as one unit; the handlers that used to take the lock directly now call `syscall_socket_lock()`/`_unlock()` and read `syscall_socket_total_connections()`, `_capacity()` and `_per_port_limit()`. `syscall_release_process_resources` moved with it, because it walks the same rows. Verified by `make compile-check`, the ABI contract and `make qemu-network-full-gate`. Fourth module landed: `userspace/lib/control_render_primitives.c` (493 lines) takes the control client's formatting and parsing primitives out of `userspace/lib/xaios_control_client.c`, which drops from 3589 to 3099. They own no state, so only prototypes crossed, into a new `xaios_control_internal.h`. The install verb stayed in the main file, which `boot-media-gate.py` reads. Verified by `make compile-check`, `make hosted-test` and `make qemu-smoke`. Third module landed: `kernel/fs/xbfs_util.c` (198 lines) takes the shared helpers out of `kernel/fs/xaiboot_fs.c`, which drops from 3890 to 3699. They own none of the filesystem's state, so nothing had to be exposed. Verified by `make compile-check`, `make hosted-test` (the host FAT/xaiFS suites link it) and `make qemu-filesystem-gate`. First module landed: `kernel/runtime/network_stack_wire.c` (431 lines) takes the wire-format helpers out of `kernel/runtime/network_stack.c`, which drops from 5900 to 5488. These are the functions that touch none of the stack's tables or counters -- the byte-order readers, the Ethernet/IPv4/IPv6/UDP/TCP parsers and the TCP option parse/build -- so the split moves no state and crosses one type (`network_tcp_flow_t`), which is why `build_tcp_options` stayed behind. Second module landed: `kernel/dev/nvme_completion.c` (122 lines) takes the NVMe completion path out of `kernel/dev/nvme.c`, which drops from 1701 to 1516. It touches no controller state, so only the queue and completion types crossed, into `nvme_completion.h`. Verified by `make compile-check` and `make qemu-nvme-gate` on all four configurations. Verified by `make compile-check` (all three architectures) and `make qemu-network-full-gate`. The rest of that file needs its state behind accessors first, as its own header note says. Traps found while planning the next files, so they are not rediscovered: `tests/scripts/boot-media-gate.py` reads `userspace/lib/xaios_control_client.c` as text and asserts the install verb literals are in it, so that verb stays in that file; `check-code-scanning-contract.py` and `qemu-sshd-audit-append-gate.py` read `kernel/fs/xaiboot_fs.c` for the `extents_from_blocks` text and the `XBFS_*_METADATA_SECTORS` defines; `kernel/dev/virtio/virtio_net.c` would have to export `g_net` to split its self-test, so its `rx`/`tx` paths come first; and `userspace/sshd/sshd.c` and `userspace/lib/xaios_control_client.c` have no private header yet, so each needs one before its first extraction. **Exit criterion:** the baseline is empty -- every source file is at or under 500 lines -- with each split landing on its own and verified by `make compile-check` plus the gates for its area (hosted tests for `userspace/` and `engine/`, the QEMU gates for `kernel/`), while the ratchet stays in place so the debt can only shrink. Split order, largest first by area: `kernel/runtime/` (`network_stack.c`, `remote_login.c`), `kernel/fs/`, `userspace/lib/`, `userspace/sshd/`, `tests/`, then the rest. |
