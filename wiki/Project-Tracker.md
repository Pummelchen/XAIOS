...
| D2 | Network boot for blank machines | `TESTING` | A machine boots from a medium holding the loader and nothing else -- no `kernel.elf`, no `initfs.img` -- installs XAIOS onto the blank disk in front of it, and that disk boots on its own to a login prompt with SSH listening. `make qemu-netboot-gate` runs all three and treats only the third as evidence. `scripts/build-netboot-image.sh` appends the kernel, the initial filesystem, an entropy seed and an unmodified copy of the loader to the loader as PE sections, and the loader finds them by parsing the headers of the image firmware mapped for it. That is what network boot requires: firmware fetches one file and is done, so the file has to be the whole system. An ordinary loader on an EFI System Partition has no such sections and reads its files exactly as before. The copy of the loader is carried because a netbooted machine cannot write the binary it is running -- firmware maps a PE with its sections at their virtual addresses, so the image in memory is not the file it came from, and writing it back out faults the firmware before any XAIOS code runs. What it writes instead is an ordinary EFI System Partition: a machine installed over the network is not a special kind of machine afterwards. What is untested is the fetch. The EDK2 build shipped with QEMU here has no network boot drivers compiled in, so the gate delivers the file on a medium rather than over TFTP. That substitutes the transport and keeps everything that depends on the file being self-contained, which is XAIOS's half; serving it from a DHCP boot filename is the firmware's, and a firmware with those drivers serves this file unchanged. To close it: run the gate against a firmware built with NetworkPkg, or a physical machine that PXE boots. |
...
| E4 | Multiqueue and RSS networking | `IN PROGRESS` | The blocker recorded here has dissolved, and not by work aimed at it. E4 was held to be waiting on moving ARM64 virtio-net from virtio-MMIO to PCI, because per-queue interrupts need MSI-X and MMIO has one interrupt line. The transitional-device-ID fix made for the installer removed that: QEMU presents `virtio-net-pci` with a transitional identity by default, which the matcher used to reject, and with it accepted ARM64 now binds virtio-net over PCI unchanged -- measured, `virtio-net: modern PCI transport index=2` with `MSI-X queue=0 vector=8193 enabled` and the queue, parser and reset self-tests passing over that transport. No transport work is needed. What is actually missing is smaller than the blocker suggested and still real. Both queues share one MSI-X vector, because `configure_msix` returns early once a device has an interrupt configured, so every queue is programmed to vector 0 -- one vector cannot steer anything. And there is no multiqueue negotiation at all: no `VIRTIO_NET_F_MQ`, no control virtqueue, no `max_virtqueue_pairs`, and `setup_queue` is called for queues 0 and 1 and nowhere else. The order is: give each queue its own MSI-X table entry and LPI; negotiate `VIRTIO_NET_F_MQ` and `VIRTIO_NET_F_CTRL_VQ`; set up N pairs plus the control queue; send `VIRTIO_NET_CTRL_MQ_VQ_PAIRS_SET`. RSS is a further step and needs `VIRTIO_NET_F_RSS` on top of that. The first of those touches the interrupt path every virtio device shares, which is the part to be careful with rather than the part that is hard. |
