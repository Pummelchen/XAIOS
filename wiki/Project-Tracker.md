...
| C-03 | The socket path does not scale across cores | `OPEN` | Measured on eight real cores under Virtualization.framework with `/bin/perfbench`. A syscall costs 2,649 ns at one thread, 4,516 at four and 5,819 at eight -- 2.2x the cost for 8x the threads, which is contention overhead on a path that genuinely runs in parallel. Socket bind/close costs 97,642 ns at one thread and 437,018 at four, and going from four threads to eight moves nothing: 171 ms of wall time for twice the work. Flat wall time against rising thread count is the signature of a serialised section, and the section is the coarse network guard from C-01. Correctness was the right thing to buy first and this is the bill. Fixing it is C-02: the guard cannot be made finer while every layer of the stack touches the same file-scope state, so the state has to be encapsulated before the lock can be split. Nothing here is a performance claim under the benchmark contract; it is a hypervisor on one eight-core machine. |
