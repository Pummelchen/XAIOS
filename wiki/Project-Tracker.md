...
| E5 | xaibootFS and xaiFS at scale | `NOT STARTED` | Not attempted, and the reason is now measured rather than asserted. xaibootFS addresses blocks with 16-bit numbers, which caps a volume at 32 MiB, and v5 gives 512 direct blocks per file for a 256 KiB maximum. The obvious fix -- widen the block number and raise the counts -- does not work, because the design holds the entire node table in memory: a node carries its block list and a snapshot copy inline, so at 32-bit block numbers a table of 4096 nodes with 8192 blocks each is **257 MiB resident**, on machines this runs with 2 GiB. At 16384 nodes it is 8 GiB. The table is the constraint, not the field width. Reaching scale therefore means extents instead of direct block lists, and metadata read on demand instead of a resident table -- a different on-disk format and a different access path, not a version bump. The file already carries v2 through v5 side by side, so the versioning mechanism to add one is there; what is not there is the design. It was left alone deliberately. xaibootFS holds every piece of system state this machine has -- SSH host keys, sshd configuration, the lifecycle record, boot counters -- and a format change that is half-finished loses all of it. Nothing else on this board risks that. xaiFS is separately tested to 128 GiB sparse, which derisks xaiFS and says nothing about xaibootFS. The shape of the fix is settled even though the work is not. An extent -- a start and a length -- describes a run of blocks in eight bytes however long the run is, so sixteen of them describe a file of any size the volume holds while making the node smaller rather than larger. With a bit per block instead of a byte, a 1 GiB volume costs a 256 KiB bitmap and 1024 nodes come to roughly 600 KiB: about 850 KiB resident for a 256-fold increase over v5's 4 MiB of data. An attempt to land it incrementally established one thing worth recording: it cannot be. The build rejects unused functions, so nothing compiles until format, mount, allocation, read, write and fsck all exist together -- there is no harmless partial state, and the vertical slice is the unit of work. That slice, plus migration proven both ways and tests, is a working session of its own rather than the tail of another. To close it: design the extent format, add it as v6 beside the existing versions, implement mount, read, write, fsck and journal against it, and prove migration from v5 both ways before anything writes v6 by default. |
