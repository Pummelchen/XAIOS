...
| `make qemu-nvme-gate` | Async PRP/SGL, direct-buffer, cancellation, malformed-completion, stress and backing-byte checks on four rows: four queues with every-queue MSI-X/LPI delivery on AArch64 and x86_64, and RISC-V twice, on both of its boards, required to disagree. QEMU's default `virt` publishes a PLIC, which carries wires and no messages, so its single queue is polled and the row says so positively -- `msix=0` and the skipped self-test -- because a build that quietly stopped configuring interrupts everywhere would otherwise pass by looking exactly like it. `virt,aia=aplic-imsic` publishes an APLIC and an IMSIC, and there the same kernel image must program a vector and be handed its completion by interrupt (`controller=aia-imsic`). The queue count stays one on both, which is an SMP bring-up property and not an interrupt one: the driver asks for one queue per online CPU and the secondary harts are still at the scheduler rendezvous when NVMe initialises (`P-16`). It also requires the stress phase's own margin line, and requires that the line account for both kinds of wait the phase performs -- the batched wait for a round, and the single-request wait that the flush and the cancellation return through -- because that margin is the only evidence there is about `B-100`, and a driver that stopped reporting it, or reported only one of the two kinds, would otherwise pass here in silence. It also requires the controller-restart self-test -- the recovery `B-100` needs -- so a reset path that stopped being exercised on a healthy boot would fail here rather than waiting for a failure to notice. The margin is required to be *reported*, not bounded: on a shared host the spread between runs belongs to the host, and failing a loaded machine here would be the shape of the defect the margin exists to explain rather than a check on it. |
