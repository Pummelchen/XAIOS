...
| `make qemu-network-poll-cadence-gate` | B-44 and OD-011: how long the network stack can go unpolled. Most of the poll comes from the syscalls sshd makes; one secondary CPU carries a network tick whose interrupt wakes it and whose idle loop polls the stack, which bounds the window a blocking call opens. Measured: **24 ms idle, 29 ms under load**, against 55-93 ms and 164-297 ms before the tick, with several thousand tick polls per run and zero gaps past a second. The gap is *recorded* rather than asserted, because on a shared host the spread between runs belongs to the host; what is asserted is that the instrument works, the poll count advances, every transfer came back byte-identical, and — the assertion that matters for the tick — that a guest which announces an armed tick shows the tick poll count advancing. It also names each self-test the boot accepted as refused rather than run. |
...
| `make qemu-x86_64-smoke` | x86_64 boot on the QEMU profile CI runs: UEFI handoff, the AP bring-up barrier, the common runtime, every userland surface by name (`/bin/smptest`, `/bin/nettest`, both shells, `sysinfo`, `agenttest`, the C99 probes), sshd, and SSH readiness. It requires two self-tests by name, and both build the thing they test rather than waiting for it. The shootdown-acknowledgement one (`B-123`) has one CPU spin on a reentrant guard while the CPU holding it shoots down, so the answer cannot arrive as an interrupt; its verdict carries whether the kernel completed it, whether the kernel as it was -- the spin's answer suppressed -- timed out, and whether the answer came from the spin rather than from an interrupt that landed before the spin masked interrupts. The idle-wakeup one (`B-120`) widens the gap between an idle CPU's check for work and its halt, hands it a thread while it is inside that gap, and then runs the same construction with the halt written the way it was, which must lose the thread. Where the machine has one CPU neither can run, and the gate requires the named refusal there instead: a check that could not be run and a check that passed must not look the same. |
| `make qemu-smoke` | Primary AArch64 boot and deterministic self-test gate. Two of the markers it requires are self-tests the guest *refuses* rather than runs -- no PCI NVMe controller, and a lease it cannot exercise -- and it requires them because a refusal is a result: a build that quietly stopped configuring interrupts everywhere would otherwise pass by looking like a machine with nothing to configure. It now names each refusal it accepted on the way out, so a run with every self-test executed and a run with two refused are different sentences in the log rather than the same silence. Where the console did lose a line, the note a failing gate prints says which context lost it (`in_handler=N masked=N`), because a drop inside a handler is the short wait working as designed and a drop outside one is the budget still being wrong. Both of its architectures also require the line that says the two x86-64 checks do not apply to them -- AArch64's `tlbi` is broadcast and RISC-V's remote fence is a firmware call that returns when every hart has fenced, and both close the idle-wakeup window by construction rather than with a re-check -- because a check that silently disappears looks exactly like one that passed. |
...
| `make qemu-nvme-gate` | Async PRP/SGL, direct-buffer, cancellation, malformed-completion, stress and backing-byte checks on four rows: four queues with every-queue MSI-X/LPI delivery on AArch64 and x86_64, and RISC-V twice, on both of its boards, required to disagree. QEMU's default `virt` publishes a PLIC, which carries wires and no messages, so its single queue is polled and the row says so positively -- `msix=0` and the skipped self-test -- because a build that quietly stopped configuring interrupts everywhere would otherwise pass by looking exactly like it. `virt,aia=aplic-imsic` publishes an APLIC and an IMSIC, and there the same kernel image must program a vector and be handed its completion by interrupt (`controller=aia-imsic`). The queue count stays one on both, which is an SMP bring-up property and not an interrupt one: the driver asks for one queue per online CPU and the secondary harts are still at the scheduler rendezvous when NVMe initialises (`P-16`). `B-100` is closed -- the completion it kept losing was one the driver read while the device was still writing it, and the reads are one access with the phase checked first now -- so the margin and the restart self-test are no longer the evidence for an open defect; they remain because they are what says the phase met its budget and the recovery path still works. It requires the stress phase's own margin line, and requires that the line account for both kinds of wait the phase performs -- the batched wait for a round, and the single-request wait that the flush and the cancellation return through -- because that margin is the only evidence there is about `B-100`, and a driver that stopped reporting it, or reported only one of the two kinds, would otherwise pass here in silence. It also requires the controller-restart self-test -- the recovery `B-100` needs -- so a reset path that stopped being exercised on a healthy boot would fail here rather than waiting for a failure to notice. The margin is required to be *reported*, not bounded: on a shared host the spread between runs belongs to the host, and failing a loaded machine here would be the shape of the defect the margin exists to explain rather than a check on it. |
...
| `make qemu-nvme-stress-soak` | The harness `B-100` was found with, and the way its fix is stated rather than assumed: 40 loaded runs with zero reproductions is what closed the row, and it is how a regression would be caught. Not a gate: the row the defect was seen on, booted repeatedly with the host held under eight busy loops, because the defect appears in about two runs in ninety and one run therefore tells you nothing. Each run is `qemu-nvme-gate` itself, invoked for that row, so a reproduction is also the gate failing rather than a second definition of the boot; the consoles are kept and the margin each boot measured is reported. A run that fails *without* `nvme: self-test failed` is counted apart, because a healthy guest that never reached its markers inside the gate's deadline is load sensitivity in the harness and not this defect. A run that *does* reproduce is stopped as soon as that line appears, because the gate would otherwise spend its whole deadline waiting for markers a failed self-test never prints -- the eight-minute failure this row keeps having to explain. |
...
| `make qemu-operations-closure` | The administrative lifecycle on every architecture this project runs on: boot, leave a durable lifecycle record by stopping QEMU abruptly, boot again and be told the previous boot was unclean, `reboot` from an SSH session, drive `ifconfig`, `route`, `netstat`, `limits`, the service manager, updates, clock and NTP, ping, the resolver's four distinct answers, config export/import, the support bundle, `kill 1` refused, and an orderly `shutdown` whose next boot reports `unclean_boots=0` -- all over SSH from this host, plus the same from a Debian container. It runs on aarch64, x86_64 and riscv64; RISC-V was added after the objective's own wording named all three architectures and this gate was the only one still confined to two, so its evidence for the third was a set of neighbouring gates rather than this lifecycle. |
