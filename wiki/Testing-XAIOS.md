...
| `make qemu-riscv64-gate` | RISC-V rv64gc on the QEMU `virt` board: boots the shared kernel to the first-run setup prompt across four harts and requires PCI enumerated, a virtio disk that reported its capacity, the initial filesystem mounted, `/init` and the service manager run and returned from, and at least 70 self-tests passed, with no assertion or panic anywhere in the boot. Four harts deliberately: firmware picks its own boot hart and it is not always hart 0, so every run draws again. Negative control run -- zeroing the filesystem header fails it on six markers. |
