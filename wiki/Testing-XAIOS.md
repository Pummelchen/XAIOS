...
| `make qemu-x86_64-smoke` | x86_64 boot on the QEMU profile CI runs: UEFI handoff, the AP bring-up barrier, the common runtime, every userland surface by name (`/bin/smptest`, `/bin/nettest`, both shells, `sysinfo`, `agenttest`, the C99 probes), sshd, and SSH readiness. It requires the shootdown-acknowledgement self-test by name (`B-123`), because that check builds a deadlock on purpose: one CPU spins on a reentrant guard while the CPU holding it shoots down, so the answer to the shootdown cannot arrive as an interrupt. Its verdict carries whether the kernel completed it, whether the kernel as it was -- the spin's answer suppressed -- timed out, and whether the answer came from the spin rather than from an interrupt that landed before the spin masked interrupts. |
| `make qemu-smoke` | Primary AArch64 boot and deterministic self-test gate. Two of the markers it requires are self-tests the guest *refuses* rather than runs -- no PCI NVMe controller, and a lease it cannot exercise -- and it requires them because a refusal is a result: a build that quietly stopped configuring interrupts everywhere would otherwise pass by looking like a machine with nothing to configure. It now names each refusal it accepted on the way out, so a run with every self-test executed and a run with two refused are different sentences in the log rather than the same silence. Both of its architectures also require the line that says the x86-64 shootdown-acknowledgement check does not apply to them -- AArch64's `tlbi` is broadcast and RISC-V's remote fence is a firmware call that returns when every hart has fenced -- because a check that silently disappears looks exactly like one that passed. |
