...
| `/bin/wtqtest` | The XAIOS client end of the WebTransport handshake gate (B-131). It connects to a host peer at `10.0.2.2:4433` over QEMU's user network, pins the fingerprint of the certificate that peer presents, and prints `wtqtest: WT-HANDSHAKE-OK` once the QUIC and TLS 1.3 handshake is complete and the peer's transport parameters are in force. It is a hosted application built with the vendored WebTransport C99 library, this repository's BearSSL backend, and the XAIOS socket seam; it is packaged in every image but launched only by `XAIOS_WT_HANDSHAKE_TEST`, because an ordinary boot has no peer to talk to and should not wait for one. |
...
| `/bin/joinnest` | Enters B-02's window on purpose: a helper thread takes a worker CPU, a victim is pinned to the same CPU, and the helper joins it, so the join can only return by running that thread nested inside its own syscall. Checks the borrowed CPU came back by using a capability and a user pointer afterwards. |
...
| `/bin/netsocktest` | Asks the kernel for datagram sockets it names itself (`xaios_net_open_udp` with port zero), then checks that each port comes back real, inside the dynamic range and different from the last; that a port asked for by number comes back unchanged; and that a datagram leaves from the socket that was given one. Reports counts and asserts nothing, so a bad number is the gate's verdict rather than a boot failure. |
