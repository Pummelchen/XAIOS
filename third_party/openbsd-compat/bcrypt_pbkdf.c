...
/* Adapted for the freestanding XAIOS SSH client. */
...
bcrypt_hash(uint8_t *sha2pass, uint8_t *sha2salt, uint8_t *out)
{
	...
	secure_zero(cdata, sizeof(cdata));
    ...
    uint8_t *key, size_t keylen, unsigned int rounds)
{
	...
	if (passlen == 0 || saltlen == 0 || keylen == 0 ||
	    keylen > sizeof(out) * sizeof(out) || saltlen > 1024U)
		goto bad;
	stride = (keylen + sizeof(out) - 1) / sizeof(out);
	amt = (keylen + stride - 1) / stride;

	copy_bytes(countsalt, salt, saltlen);
	...
	for (count = 1; keylen > 0; count++) {
		...
		sha512_hash(countsalt, saltlen + 4U, sha2salt);

		bcrypt_hash(sha2pass, sha2salt, tmpout);
		copy_bytes(out, tmpout, sizeof(out));
	...
	/* zap */
	secure_zero(countsalt, sizeof(countsalt));
	secure_zero(out, sizeof(out));
	secure_zero(tmpout, sizeof(tmpout));
	secure_zero(sha2pass, sizeof(sha2pass));
	secure_zero(sha2salt, sizeof(sha2salt));
