...
| DNS | The asynchronous resolver supports A/AAAA results, TTL-bounded caching, retry/timeout, and DNS-over-TCP fallback. Queries set EDNS DO and CD; XAIOS locally validates DNSKEY, DS, and RRSIG chains from compiled root DS anchors and never treats an upstream AD bit as authority. RSA/SHA-256, ECDSA P-256/P-384, Ed25519, SHA-256/SHA-384 DS, and signed exact-owner NSEC NODATA are supported. Unsupported NXDOMAIN, NSEC3, CNAME/DNAME or wildcard synthesis, an untrusted clock, malformed data, and unsupported algorithms fail closed. The hosted signed-chain test covers corruption, expiry, and hostile parser input; root-anchor rollover policy and physical deployment remain open. |
