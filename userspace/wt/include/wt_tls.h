...
typedef struct wt_tls_traffic_keys {
  ...
  /* How many bytes of `hp` are meaningful: 16 for AES-128-GCM, 32 for
     ChaCha20-Poly1305. Header protection must use exactly this many. */
  size_t hp_len;
 ...
 * (or, on a resumed handshake, through the server's first flight end), which
 * is where the application traffic secrets and the exporter are taken. A
 * caller that has no transcript yet can pass wt_tls_empty_hash for both, which
 * is what the RFC 8448 vectors do at the points they are printed for; that is
 * useful for testing the schedule and wrong for a connection.
 *
 ...
 * 0 when it does not, and -1 on a bad argument. */
