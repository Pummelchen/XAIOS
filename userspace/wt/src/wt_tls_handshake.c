/* TLS 1.3 handshake framing, the transcript hash, ServerHello parsing, and the
 * Finished MAC.
 *
 * RFC 8446 sections 4, 4.1.3 and 4.4.4. See wt_tls_handshake.h for the scope.
 * The EncryptedExtensions parser and the ClientHello builder live in
 * wt_tls_handshake_ee.c and wt_tls_handshake_clienthello.c; the body cursor and
 * the version constants they share are in wt_tls_handshake_internal.h, and the
 * cursor is defined here.
 */
...
void wt_tls_reader_init(wt_tls_reader_t *r, const uint8_t *data,
                        size_t len) {
  r->data = data;
  r->len = len;
  r->offset = 0U;
...
uint8_t wt_tls_reader_u8(wt_tls_reader_t *r) {
  const uint8_t *p = wt_tls_reader_take(r, 1U);
  return p == NULL ? 0U : p[0];
}

uint16_t wt_tls_reader_u16(wt_tls_reader_t *r) {
  const uint8_t *p = wt_tls_reader_take(r, 2U);
...
int wt_tls_parse_server_hello(const uint8_t *message, size_t message_len,
  ...
  wt_tls_reader_init(&r, body, body_len);
  ...
  if (wt_tls_reader_u16(&r) != WT_TLS_LEGACY_VERSION) return -1;
  ...
  /* random: 32 bytes, which this parser does not use. A HelloRetryRequest is
     ...
     ServerHello. */
  {
    const uint8_t *random = wt_tls_reader_take(&r, 32U);
  ...
  {
    uint8_t echo_len = wt_tls_reader_u8(&r);
    const uint8_t *echo = wt_tls_reader_take(&r, echo_len);
  ...
  out->cipher_suite = wt_tls_reader_u16(&r);

  /* legacy_compression_method must be zero in TLS 1.3. */
  if (wt_tls_reader_u8(&r) != 0U) return -1;

  /* extensions<6..2^16-1>: a two-byte length then that many bytes. */
  {
    uint16_t extensions_len = wt_tls_reader_u16(&r);
    const uint8_t *extensions = wt_tls_reader_take(&r, extensions_len);
    wt_tls_reader_t ext;
    if (extensions == NULL) return -1;
    /* RFC 8446 requires at least one extension in a ServerHello. */
    if (extensions_len < 6U) return -1;
    wt_tls_reader_init(&ext, extensions, extensions_len);
    while (ext.offset < ext.len) {
      uint16_t ext_type = wt_tls_reader_u16(&ext);
      uint16_t ext_len = wt_tls_reader_u16(&ext);
      const uint8_t *ext_data = wt_tls_reader_take(&ext, ext_len);
      ...
      if (ext_type == WT_TLS_EXT_SUPPORTED_VERSIONS) {
        ...
        wt_tls_reader_t ks;
        uint16_t key_len;
        wt_tls_reader_init(&ks, ext_data, ext_len);
        out->group = wt_tls_reader_u16(&ks);
        key_len = wt_tls_reader_u16(&ks);
        out->key_share = wt_tls_reader_take(&ks, key_len);
