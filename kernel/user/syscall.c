    ...
    if (socket_snapshot.state == KERNEL_SOCK_DATAGRAM &&
      ...
      if (request.port == 0U || request.port > UINT16_MAX ||
          vmm_validate_user_buffer(request.addr_ptr, sizeof(destination), 0) !=
              XAIOS_OK) {
        return reject_syscall(syscall, arg0, arg1, "net-sendto-denied");
      }
      bytes_copy(&destination, (const void *)(uintptr_t)request.addr_ptr,
                 sizeof(destination));
      if (destination.family != XAIOS_IP_FAMILY_V4) {
        return reject_syscall(syscall, arg0, arg1, "net-sendto-family");
      }
      uint8_t *datagram = (uint8_t *)kheap_alloc(request.buffer_size, 16U);
      if (datagram == 0) {
        return reject_syscall(syscall, arg0, arg1, "net-sendto-no-memory");
      }
      bytes_copy(datagram, (const void *)(uintptr_t)request.buffer,
                 request.buffer_size);
