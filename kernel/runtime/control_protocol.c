...
/* control_storage_ops.c and control_ops.c share these helpers through the
   private header. The forwarding keeps the file-local names unexported:
   string_equal in particular is already a global symbol in remote_login.c. */
...
/* The log helpers the self-test exercises stay here; control_ops.c reaches
   them through the private header so handle_logs and log_level read as they
   always did. */
