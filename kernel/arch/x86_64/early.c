 ...
 * call sites -- x86_64_kmain here and x86_64_ap_entry, now in early_ap.c --
 * spelling it the way they did. The BSP's rsp0 is reached through the scalar
 * accessors the platform hooks below call, never through a pointer into that
 * file's state. */
 ...
 * reload uses -- that reload now lives in early_ap.c, which carries its own
 * alias -- spelling them the way they did. The module defines the object
 ...
 * early_lapic.c; the aliases below keep the call sites that remain here -- the
 * LAPIC timer self-test -- spelling them the way it did, while the AP startup
 * sequence now in early_ap.c carries its own aliases. The raw MSR/CR/TSC/CPUID
 * primitives are `static inline` in early_lapic.h, so the call sites that
 * inline them are unchanged. */
 ...
 * live in early_fpu.h, which the AVX2 canary in x86_64_kmain and the AP entry
 * in early_ap.c share with early_fpu.c. MSR_IA32_APIC_BASE moved to
 ...
 * self-test here and the AP entry in early_ap.c still name the same constants,
 * which now come from that header. */
 ...
 * early_module.h: early_gdt.c sizes the BSP syscall stack with them and
 * early_ap.c guards the kernel and syscall stacks it allocates with them. */
 ...
 * user; early_ap.c still reads that layout through the header, because its
...
/* The ten x86_64_ap_trampoline_* entry.S symbols moved to early_ap.c with
 * patch_ap_trampoline(), their only reader. */

/* g_idt moved to early_idt.c as the exported g_x86_idt; the AP entry that
 * reached the table through the alias above, for its IDTR reload, moved to
 * early_ap.c, which carries its own alias. */
...
/* The CPU record table and these three scalars cross to early_ap.c, whose AP
 * bring-up reads and writes them; their storage stays here, only the
 * file-scope `static` is dropped, and early_ap.h declares the objects and
 * keeps every statement on both sides spelling them the way this file always
 * did. */
x86_64_cpu_record_t *g_cpu_records;
uint32_t g_cpu_record_count;
uint32_t g_bsp_ordinal = UINT32_MAX;
volatile uint32_t g_common_worker_release;
 ...
 * setter and getter; the AP idle loop that reads it through
 * xaios_x86_early_idle_probe_get now lives in early_ap.c. */
 ...
 * still inlines them at the AVX2 canary and early_ap.c at x86_64_ap_entry; the
 * two save/restore entry points keep their names because entry.S calls them. */

/* The MSR/CR/TSC/CPUID primitives that stood here now live, also `static
 * inline`, in early_lapic.h: this file still inlines rdmsr and rdtsc in the
 * LAPIC timer self-test and cpuid at the AVX2 canary, early_ap.c inlines
 * read_cr3, rdtsc and the AP startup delays, and early_lapic.c inlines the
...
/* bytes_copy moved to early_ap.c with patch_ap_trampoline(), its only
 * caller. */
 ...
 * g_tss and g_syscall_stack. The aliases at the top of this file keep the call
 * site in x86_64_kmain, and early_ap.c carries the AP entry's own aliases,
 * spelling them as before. */
 ...
 * top of this file keep the call sites that remain here -- the LAPIC timer
 * self-test -- spelling them the way it did; early_ap.c carries its own
 * aliases for the AP startup sequence, and the raw primitives come from
...
/* tsc_delay() and x86_64_ap_entry() moved to early_ap.c together with the
 * trampoline patch and the whole start_application_processors() sequence.
 * Every statement moved verbatim -- the `lidt`, the CR4/XCR0 programming, the
 * LAPIC spurious-vector write, the store that marks the CPU online and the
 * `sti; hlt` idle pair with the `cli`/`sti` ordering around it -- so nothing
 * about the AP entry, its barriers or its delays changed. The entry.S
 * trampoline still reaches the same function through the same absolute
 * address, patched at the same point in x86_64_kmain. */
...
/* The trampoline patch -- write_u32(), write_u64() and
 * patch_ap_trampoline(), with its page copy, its absolute entry address and
 * the `mfence` after it -- and the whole start_application_processors()
 * sequence, including the BSP lookup, the per-AP stack allocation and guard
 * fill, the INIT-SIPI-SIPI writes with their tsc_delay() waits and the legacy
 * IPI-worker self-test under !XAIOS_X86_COMMON_RUNTIME, all moved verbatim to
 * early_ap.c. x86_64_kmain calls the sequence at this same point, through the
 * start_application_processors alias in early_ap.h. */
