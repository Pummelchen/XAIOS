/* x86-64 SMP self-tests, split out of smp.c.
 *
 * Both of these build a cycle that was hit in the field and then run the half
 * that is the kernel before the fix as a negative control: a CPU that cannot
 * take an interrupt still has to answer a TLB shootdown (B-123), and a wakeup
 * must not be lost between the idle loop's check and its halt (B-120). They
 * drive x86-64 platform probe hooks, so they live beside the file they came
 * from. The two symbols they define -- smp_shootdown_ack_self_test and
 * smp_idle_wakeup_self_test -- are already declared in kernel/include/xaios/
 * smp.h and called from kernel/core/boot_runtime.c, so nothing here needs a
 * new private header: only the public declarations are shared.
 */
