...
static xaios_status_t mutable_open(void *context, const char *path,
  ...
  (void)context;
  int64_t fd = xaiboot_fs_open(path, flags);
...
static xaios_status_t mutable_close(void *context, uint64_t handle) {
  (void)context;
  return handle <= UINT32_MAX ? xaiboot_fs_close((uint32_t)handle)
                              : XAIOS_ERR_INVALID;
}
...
static int64_t mutable_pread(void *context, uint64_t handle, void *buffer,
  ...
  if (handle > UINT32_MAX ||
      xaiboot_fs_seek((uint32_t)handle, offset) != XAIOS_OK) {
    return XAIOS_ERR_INVALID;
  }
  return xaiboot_fs_read_fd((uint32_t)handle, buffer, length);
}
...
static int64_t mutable_pwrite(void *context, uint64_t handle,
  ...
  return xaiboot_fs_write_fd((uint32_t)handle, buffer, length);
}

static xaios_status_t xaiboot_fsync(void *context, uint64_t handle) {
  (void)context;
  (void)handle;
  /* xaibootFS flushes each metadata/data mutation before returning. */
  return XAIOS_OK;
}
...
static xaios_status_t mutable_stat(void *context, const char *path,
  ...
  (void)context;
  xaios_xbfs_stat_t source;
  xaios_status_t status = xaiboot_fs_stat(path, &source);
...
static xaios_status_t mutable_list(void *context, const char *path,
                                   ...
                                   uint64_t *out_size) {
  (void)context;
  return xaiboot_fs_list(path, buffer, capacity, out_size);
}
