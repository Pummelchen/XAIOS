...
sized XSAVE state and operates modern VirtIO block/MSI-X plus network TX; full
x86 platform and userspace/service parity remains incomplete. The authoritative
20-item boundary is `docs/PLATFORM-SUPPORT.json`.
...
| AI runtime | `kernel/runtime/cpu_ai_runtime.c`, `kernel/runtime/model_arena.c`, `kernel/runtime/ai_cell.c` | CPU-only fixture runtime, fixture-copy arena, no-copy immutable 64-bit mappings, and resource isolation. |
| Portable engine | `engine/include/xaios_engine/`, `engine/src/` | model-v2/ModelFS parsing, architecture registry, caller-owned service/model/session lifecycle, direct async range I/O, backend API, scalar projection, and no-expand INT4/INT6 scalar plus experimental NEON/AVX2 GEMV/GEMM. |
...
- Production model admission retains immutable readers and no-copy 64-bit kernel
  mappings. The copied model-v1 arena API is explicitly fixture-only.
